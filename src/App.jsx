...
export default function App() {
  ...
  // ---------- Trazabilidad: reconstruye el flujo completo de documentos alrededor de una orden ----------
  const extraerFolioOrden = (texto) => texto?.match(/OT-\d{4}-\d{3}/)?.[0] || null;
  const [trazaOrden, setTrazaOrden] = useState(null); // folio de la orden cuyo flujo se está viendo, o null si el panel está cerrado
  const trazaDeOrden = (folio) => {
    const orden = orders.find((o) => o.id === folio);
    ...
    return { orden, asientos: asientosRel, kardex: kardexRel };
  };
  ...
  const addCxp = async (data) => {
    ...
    setCxp((p) => [...p, { id: nextId('CP', p, 5), supabaseId: nuevo.id, estado: 'pendiente', ...data, monto }]);
  };
  ...
  const addNomina = async (data) => {
    ...
    setNomina((p) => [...p, { id: nextId('NM', p, 5), supabaseId: nueva.id, estado: 'pendiente', ...data, montoBruto: bruto, descuentoAnticipo, descuentoPrestamo, monto: neto }]);
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', backgroundColor: C.bg, minHeight: '100vh', display: 'flex' }}>
      ...
      <div className="flex-1 p-4 md:p-8" style={{ maxWidth: 1220, minWidth: 0, overflowX: 'hidden' }}>
        ...
        {trazaOrden && (() => {
          const { orden, asientos: aRel, kardex: kRel } = trazaDeOrden(trazaOrden);
          const pasos = [
            { tipo: 'orden', color: C.cyan, titulo: `Orden ${trazaOrden}`, sub: orden ? `${orden.fecha} · ${empleadoName(orden.tecnicoId)}` : 'creada', doc: orden },
                        ...
                        <React.Fragment key={a.id}>
                          ...
                          <tr style={{ opacity: opacidad, textDecoration: tachado }}>
                            ...
                            <td rowSpan={4} style={{ verticalAlign: 'top', paddingTop: 6, whiteSpace: 'nowrap' }}>
                              {!a.anulado && (
                                <div className="flex gap-2">
                                  {extraerFolioOrden(a.glosa) && <button onClick={() => abrirTrazabilidad(extraerFolioOrden(a.glosa))} title="Ver trazabilidad"><GitBranch size={14} color={C.cyanDark} /></button>}
