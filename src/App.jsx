...
export default function App() {
  ...
  const extraerFolioDocumento = (texto) => texto?.match(/OT-\d{4}-\d{3}|VT\d{5}|CP\d{5}|NM\d{5}/)?.[0] || null;
  const extraerFolioOrden = (texto) => texto?.match(/OT-\d{4}-\d{3}/)?.[0] || null;
  const [trazaOrden, setTrazaOrden] = useState(null); // folio del documento raíz cuyo flujo se está viendo, o null si el panel está cerrado
  const trazaDeOrden = (folio) => {
    const tipo = folio.startsWith('OT-') ? 'orden' : folio.startsWith('VT') ? 'venta' : folio.startsWith('CP') ? 'cxp' : folio.startsWith('NM') ? 'nomina' : null;
    const raiz = tipo === 'orden' ? orders.find((o) => o.id === folio)
      : tipo === 'venta' ? ventas.find((v) => v.id === folio)
      : tipo === 'cxp' ? cxp.find((c) => c.id === folio)
      : tipo === 'nomina' ? nomina.find((n) => n.id === folio) : null;
  ...
  const abrirTrazabilidad = (folioOrden) => { if (folioOrden) setTrazaOrden(folioOrden); };
  // "Entrar y revisar" un documento del flujo: cierra el panel y abre ese documento en su propio módulo.
  const revisarDocumentoDeTraza = (tipo, doc) => {
    setTrazaOrden(null);
    if (tipo === 'asiento') { setTab('contab_comprobantes'); setEditingAsiento(doc); setShowAsientoForm(false); }
    else if (tipo === 'kardex') { setTab('kardex'); setKardexMaterial(doc.materialId); }
    else if (tipo === 'orden') { setTab('ordenes'); setOrdersSearch(doc.id); }
    else if (tipo === 'venta') { setTab('ventas'); }
    else if (tipo === 'cxp') { setTab('cxp'); }
    else if (tipo === 'nomina') { setTab('nomina'); }
  };
  ...
  const addFacturaManual = async (data) => {
    ...
    setFacturasManuales((p) => [...p, { id: nextId('FM', p, 5), supabaseId: nueva.id, ...data, monto, cuentaIngreso: cuentaIngresoCodigo, estadoPago: 'pendiente' }]);
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', backgroundColor: C.bg, minHeight: '100vh', display: 'flex' }}>
      ...
      <div className="flex-1 p-4 md:p-8" style={{ maxWidth: 1220, minWidth: 0, overflowX: 'hidden' }}>
        ...
        {trazaOrden && (() => {
          const { tipo: tipoRaiz, raiz, asientos: aRel, kardex: kRel } = trazaDeOrden(trazaOrden);
          const etiquetaRaiz = { orden: 'Orden', venta: 'Venta', cxp: 'Cuenta por pagar', nomina: 'Nómina' }[tipoRaiz] || 'Documento';
          const nodos = [
          ...
          return (
            <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(11,22,34,0.5)', zIndex: 65, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setTrazaOrden(null)}>
              <div onClick={(e) => e.stopPropagation()} className="rounded-xl p-4" style={{ backgroundColor: '#fff', maxWidth: '95vw', maxHeight: '85vh', overflow: 'auto', boxShadow: '0 20px 50px rgba(0,0,0,0.25)' }}>
                <div className="flex items-center justify-between mb-1">
                  <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 15, color: C.slate }}>Trazabilidad — {etiquetaRaiz} {trazaOrden}</div>
                  <button onClick={() => setTrazaOrden(null)}><X size={18} color={C.muted} /></button>
                </div>
                <div style={{ fontSize: 11.5, color: C.muted, marginBottom: 10 }}>Toca cualquier caja para entrar y revisar ese documento.</div>
                {nodos.length === 0 ? (
                  ...
                  <svg width={svgW} height={totalH} viewBox={`0 0 ${svgW} ${totalH}`} style={{ display: 'block' }}>
                    ...
                    <g onClick={() => raiz && revisarDocumentoDeTraza(tipoRaiz, raiz)} style={{ cursor: raiz ? 'pointer' : 'default' }}>
                      <rect x={rootX} y={rootY} width={boxW} height={boxH} rx="8" fill="#F0FBF9" stroke={C.cyanDark} strokeWidth="1" />
                      <text x={rootX + boxW / 2} y={rootY + 21} textAnchor="middle" style={{ fontSize: 12, fontWeight: 700, fill: C.slate }}>{trazaOrden}</text>
                      <text x={rootX + boxW / 2} y={rootY + 37} textAnchor="middle" style={{ fontSize: 10, fill: C.muted }}>{etiquetaRaiz}</text>
                    </g>
                    {nodos.map((n, i) => {
                      const y = marginY / 2 + i * (boxH + gapY);
                      return (
                        <g key={'n' + i} onClick={() => revisarDocumentoDeTraza(n.tipo, n.doc)} style={{ cursor: 'pointer' }}>
        ...
        {tab === 'cxp' && (
          <div>
            ...
            <div className="rounded-xl overflow-hidden mt-5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              <table>
                <thead><tr><th>Proveedor</th><th>Concepto</th><th>Tipo</th><th>Fecha</th><th>Vencimiento</th><th>Monto</th><th>Estado</th><th></th></tr></thead>
                <tbody>
                  ...
                  {cxp.length === 0 && <tr><td colSpan={8} style={{ color: C.muted, textAlign: 'center', padding: 24 }}>Sin cuentas por pagar registradas.</td></tr>}
                </tbody>
                {cxp.length > 0 && (
                  <tfoot><tr><td colSpan={5} style={{ fontWeight: 700 }}>Total ({cxp.length})</td><td style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 700 }}>{fmt(cxp.reduce((s, c) => s + c.monto, 0))}</td><td></td><td></td></tr></tfoot>
                )}
              </table>
              <Paginador p={cxpPag} />
            </div>
