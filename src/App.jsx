...
import {
  ...
  Building2, Truck, UserCheck, Calculator, ChevronDown, Pencil, Check, Fuel, ShieldAlert, Bell, GitBranch
} from 'lucide-react';
...
// Colores por estado de orden, usados tanto en el detalle de la orden (modal global) como en la Agenda.
...
export default function App() {
  ...
  const kardexRef = useRef([]); // evita folios KD repetidos cuando se crean varios movimientos en la misma acción
  ...
  const [proveedoresBusqueda, setProveedoresBusqueda] = useState('');
  const [materialesBusqueda, setMaterialesBusqueda] = useState('');
  const [materialesOrden, setMaterialesOrden] = useState({ campo: null, dir: 1 });
  const [cxpOrden, setCxpOrden] = useState({ campo: null, dir: 1 });
  const [buscadorGlobalAbierto, setBuscadorGlobalAbierto] = useState(false);
  const [buscadorGlobalTexto, setBuscadorGlobalTexto] = useState('');
  const [cxpBusqueda, setCxpBusqueda] = useState('');
  ...
  const [diaAgendaSel, setDiaAgendaSel] = useState(new Date().toISOString().slice(0, 10));
  const [agendaMaterialSel, setAgendaMaterialSel] = useState('');
  const [agendaMaterialCant, setAgendaMaterialCant] = useState('');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setCargandoDatos(false); // ya hay lo mínimo (cuentas/centros) para operar; el resto de módulos sigue llegando en segundo plano
      ...
      eq('materiales').then(({ data }) => setMaterials((data || []).map((m) => ({ id: m.id, nombre: m.nombre, unidad: m.unidad, costoUnitario: Number(m.costo_unitario), stock: Number(m.stock), activo: m.activo }))));
      eq('activos_fijos').then(({ data }) => setActivos((data || []).sort((a, b) => new Date(a.fecha_compra) - new Date(b.fecha_compra)).map((a, i) => ({
        id: 'AF' + String(i + 1).padStart(5, '0'), supabaseId: a.id, nombre: a.nombre, categoria: a.categoria, fechaCompra: a.fecha_compra, costo: Number(a.costo),
        valorResidual: Number(a.valor_residual), vidaUtilAnios: a.vida_util_anios, responsable: a.responsable_id,
        centroCostoId: a.centro_costo_id, placa: a.placa, km: a.km, seguroVencimiento: a.seguro_vencimiento, revisionVencimiento: a.revision_vencimiento, activo: a.activo,
      }))));
      ...
      eq('proyectos').then(({ data }) => setProjects((data || []).map((p, i) => ({ id: 'PY' + String(i + 1).padStart(5, '0'), supabaseId: p.id, nombre: p.nombre, clienteId: p.cliente_id, fechaInicio: p.fecha_inicio, presupuesto: Number(p.presupuesto), estado: p.estado }))));
      ...
      eq('ordenes').then(({ data }) => setOrders((data || []).map((o) => ({
        ...
        partidas: o.partidas || [], reprogramaciones: o.reprogramaciones || [], fotos: o.fotos || [],
      }))));
      eq('kardex').then(({ data }) => {
        const ordenado = (data || []).sort((a, b) => new Date(a.creado_en) - new Date(b.creado_en));
        const mapeado = ordenado.map((k, i) => ({
          id: 'KD' + String(i + 1).padStart(5, '0'), supabaseId: k.id, materialId: k.material_id, fecha: k.fecha, tipo: k.tipo, cantidad: Number(k.cantidad),
        ...
        kardexRef.current = mapeado;
        setKardexLog(mapeado);
      });
      eq('squadras').then(({ data }) => setSquadras((data || []).map((s, i) => ({ id: 'SQ' + String(i + 1).padStart(5, '0'), supabaseId: s.id, nombre: s.nombre, miembros: s.miembros || [], activa: s.activa }))));
      eq('equipos_comodato').then(({ data }) => setEquiposComodato((data || []).map((e, i) => ({
        id: 'EQ' + String(i + 1).padStart(5, '0'), supabaseId: e.id, tipo: e.tipo, modelo: e.modelo, serial: e.serial, sector: e.sector, estado: e.estado,
        clienteId: e.cliente_id, ordenId: e.orden_id, fechaAsignacion: e.fecha_asignacion, fechaDevolucion: e.fecha_devolucion,
      }))));
      eq('flota_log').then(({ data }) => setFlotaLog((data || []).sort((a, b) => new Date(a.fecha) - new Date(b.fecha)).map((f, i) => ({
        id: 'FL' + String(i + 1).padStart(5, '0'), supabaseId: f.id, tipo: f.tipo, activoId: f.activo_id, fecha: f.fecha, monto: Number(f.monto),
      ...
      eq('cxp').then(({ data }) => setCxp((data || []).sort((a, b) => new Date(a.fecha) - new Date(b.fecha)).map((c, i) => ({
        id: 'CP' + String(i + 1).padStart(5, '0'), supabaseId: c.id, proveedorId: c.proveedor_id, concepto: c.concepto, tipo: c.tipo, categoria: c.categoria,
        materialId: c.material_id, cantidad: c.cantidad, lineas: c.lineas, fecha: c.fecha, vencimiento: c.vencimiento, monto: Number(c.monto), estado: c.estado,
      }))));
      eq('facturas_manuales').then(({ data }) => setFacturasManuales((data || []).sort((a, b) => new Date(a.fecha) - new Date(b.fecha)).map((f, i) => ({
        id: 'FM' + String(i + 1).padStart(5, '0'), supabaseId: f.id, clienteId: f.cliente_id, concepto: f.concepto, categoria: f.categoria, fecha: f.fecha,
        monto: Number(f.monto), cuentaIngreso: codPorId[f.cuenta_ingreso_id] || '', estadoPago: f.estado_pago,
      }))));
      eq('movimientos').then(({ data }) => setMovimientos((data || []).sort((a, b) => new Date(a.fecha) - new Date(b.fecha)).map((m, i) => ({
        id: 'MV' + String(i + 1).padStart(5, '0'), supabaseId: m.id, tipo: m.tipo, categoria: m.categoria, descripcion: m.descripcion, fecha: m.fecha, monto: Number(m.monto),
      }))));

      eq('nomina').then(({ data }) => setNomina((data || []).sort((a, b) => new Date(a.fecha_pago) - new Date(b.fecha_pago)).map((n, i) => ({
        id: 'NM' + String(i + 1).padStart(5, '0'), supabaseId: n.id, empleadoId: n.empleado_id, periodo: n.periodo, fechaPago: n.fecha_pago, montoBruto: Number(n.monto_bruto),
      ...
      eq('anticipos').then(({ data }) => setAnticipos((data || []).sort((a, b) => new Date(a.fecha) - new Date(b.fecha)).map((a, i) => ({ id: 'AN' + String(i + 1).padStart(5, '0'), supabaseId: a.id, empleadoId: a.empleado_id, fecha: a.fecha, monto: Number(a.monto), motivo: a.motivo, estado: a.estado }))));
      eq('prestamos').then(({ data }) => setPrestamos((data || []).sort((a, b) => new Date(a.fecha) - new Date(b.fecha)).map((p, i) => ({
        id: 'PR' + String(i + 1).padStart(5, '0'), supabaseId: p.id, empleadoId: p.empleado_id, fecha: p.fecha, motivo: p.motivo, montoTotal: Number(p.monto_total),
      ...
      const mapeados = (asData || []).map((a) => ({
        id: 'DC' + String(a.id).replace(/-/g, '').slice(0, 6).toUpperCase(), supabaseId: a.id, fecha: a.fecha, glosa: a.glosa,
  ...
  const postAsiento = (fecha, glosa, cuentaDebe, monto, cuentaHaber, centroCostoId = null) => {
    ...
    const idLocal = 'DC' + String(asientosRef.current.length + 1).padStart(5, '0');
    const nuevoLocal = {
  ...
  // Construye el objeto local de un movimiento de Kardex con folio legible (KD-00001...) y lo registra en la referencia compartida.
  const nuevoKardexLocal = (kNuevo, datos) => {
    const folio = 'KD' + String(kardexRef.current.length + 1).padStart(5, '0');
    const obj = { id: folio, supabaseId: kNuevo.id, ...datos };
    kardexRef.current = [...kardexRef.current, obj];
    return obj;
  };

  // ---------- Trazabilidad: reconstruye el flujo completo de documentos alrededor de un documento raíz ----------
  // Se agregó FM (facturas manuales) a los tipos que se pueden seguir, además de OT/VT/CP/NM.
  const extraerFolioDocumento = (texto) => texto?.match(/OT-\d{4}-\d{3}|VT\d{5}|CP\d{5}|NM\d{5}|FM\d{5}/)?.[0] || null;
  const extraerFolioOrden = (texto) => texto?.match(/OT-\d{4}-\d{3}/)?.[0] || null;
  const tipoDeFolio = (folio) => folio?.startsWith('OT-') ? 'orden' : folio?.startsWith('VT') ? 'venta' : folio?.startsWith('CP') ? 'cxp' : folio?.startsWith('NM') ? 'nomina' : folio?.startsWith('FM') ? 'factura' : null;
  const docDeFolio = (folio) => {
    const tipo = tipoDeFolio(folio);
    const raiz = tipo === 'orden' ? orders.find((o) => o.id === folio)
      : tipo === 'venta' ? ventas.find((v) => v.id === folio)
      : tipo === 'cxp' ? cxp.find((c) => c.id === folio)
      : tipo === 'nomina' ? nomina.find((n) => n.id === folio)
      : tipo === 'factura' ? facturasManuales.find((f) => f.id === folio) : null;
    return { tipo, raiz };
  };
  const trazaDeOrden = (folio) => {
    const { tipo, raiz } = docDeFolio(folio);
    const asientosRel = asientosVigentes.filter((a) => a.glosa?.includes(folio)).sort((a, b) => a.id.localeCompare(b.id));
    const kardexRel = kardexLog.filter((k) => k.referencia?.includes(folio)).sort((a, b) => a.id.localeCompare(b.id));
    return { tipo, raiz, asientos: asientosRel, kardex: kardexRel };
  };
  // Pila de navegación del panel de trazabilidad: cada entrada es { modo: 'flujo', folio } o { modo: 'orden'|'detalle', tipo, doc }.
  // "Atrás" retrocede en la misma ventana, sin salir jamás de la pantalla ni cambiar de módulo.
  const [trazaStack, setTrazaStack] = useState([]);
  const [proveedorSel, setProveedorSel] = useState(null); // id del proveedor cuyo historial se está viendo, sin salir de la pantalla
  const trazaActual = trazaStack[trazaStack.length - 1] || null;
  const abrirTrazabilidad = (folio) => { if (folio) setTrazaStack([{ modo: 'flujo', folio }]); };
  const trazaIrA = (item) => setTrazaStack((prev) => [...prev, item]);
  const trazaAtras = () => setTrazaStack((prev) => prev.slice(0, -1));
  const cerrarTraza = () => setTrazaStack([]);
  // Al tocar una caja del flujograma (o un documento dentro de un detalle): si es una orden, se ve su ficha completa
  // (fotos, materiales, estado) aquí mismo; si es otro tipo de raíz, su tarjeta de detalle; los diarios/kardex muestran
  // su propia info y, si a su vez mencionan OTRO documento raíz, ofrecen seguir el flujo hacia allá.
  const verDocumentoEnTraza = (tipo, doc) => {
    if (!doc) return;
    if (tipo === 'orden') { trazaIrA({ modo: 'orden', doc }); return; }
    if (['venta', 'cxp', 'nomina', 'factura'].includes(tipo)) { trazaIrA({ modo: 'flujo', folio: doc.id }); return; }
    const texto = tipo === 'asiento' ? doc.glosa : tipo === 'kardex' ? doc.referencia : '';
    const otroFolio = extraerFolioDocumento(texto);
    trazaIrA({ modo: 'detalle', tipo, doc, folioRelacionado: otroFolio && otroFolio !== trazaActual?.folio ? otroFolio : null });
  };
  // ---------- Fotos de la orden (almacenadas en Supabase Storage, bucket "orden-fotos") ----------
  const subirFotoOrden = async (orden, file) => {
    if (!orden?.supabaseId || !file) return;
    const path = `${empresaActiva}/${orden.id}/${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`;
    const { error: errSubida } = await supabase.storage.from('orden-fotos').upload(path, file);
    if (errSubida) { alert('No se pudo subir la foto: ' + errSubida.message); return; }
    const { data: urlData } = supabase.storage.from('orden-fotos').getPublicUrl(path);
    const nuevaFoto = { url: urlData.publicUrl, path, subidaPor: sesion?.nombre || 'Sistema', fecha: new Date().toISOString() };
    const fotosActualizadas = [...(orden.fotos || []), nuevaFoto];
    setOrders((prev) => prev.map((o) => o.id === orden.id ? { ...o, fotos: fotosActualizadas } : o));
    supabase.from('ordenes').update({ fotos: fotosActualizadas }).eq('id', orden.supabaseId).then(({ error }) => { if (error) console.error(error); });
  };
  const eliminarFotoOrden = async (orden, foto) => {
    if (!orden?.supabaseId) return;
    pedirConfirmacion('¿Eliminar esta foto de la orden?', async () => {
      await supabase.storage.from('orden-fotos').remove([foto.path]);
      const fotosActualizadas = (orden.fotos || []).filter((f) => f.path !== foto.path);
      setOrders((prev) => prev.map((o) => o.id === orden.id ? { ...o, fotos: fotosActualizadas } : o));
      supabase.from('ordenes').update({ fotos: fotosActualizadas }).eq('id', orden.supabaseId).then(({ error }) => { if (error) console.error(error); });
    });
  };
  // Asigna un material a una orden que ya existe (no solo al crearla) — descuenta stock, registra Kardex y contabiliza el costo.
  const asignarMaterialAOrden = async (orden, materialId, cantidad) => {
    if (!orden?.supabaseId) return;
    const cant = Number(cantidad);
    const mat = materials.find((m) => m.id === materialId);
    if (!mat) return;
    if (cant <= 0) { alert('Escribe una cantidad mayor a cero.'); return; }
    if (cant > mat.stock) { alert(`No hay suficiente stock — solo hay ${mat.stock}.`); return; }
    const nuevoStock = mat.stock - cant;
    setMaterials((prev) => prev.map((m) => m.id === materialId ? { ...m, stock: nuevoStock } : m));
    ...
    const { data: kNuevo } = await supabase.from('kardex').insert({
      empresa_id: empresaActiva, material_id: materialId, fecha, tipo: 'salida', cantidad: cant, costo_unitario: mat.costoUnitario,
      referencia: `Orden ${orden.id}`, saldo_cantidad: nuevoStock, saldo_valor: nuevoStock * mat.costoUnitario, centro_costo_id: orden.centroCostoId || null,
    }).select().single();
    if (kNuevo) setKardexLog((prev) => [...prev, nuevoKardexLocal(kNuevo, {
      materialId, fecha, tipo: 'salida', cantidad: cant, costoUnitario: mat.costoUnitario,
      referencia: `Orden ${orden.id}`, saldoCantidad: nuevoStock, saldoValor: nuevoStock * mat.costoUnitario, centroCostoId: orden.centroCostoId || null,
    })]);
    const materialesActualizados = [...(orden.materialesUsados || []), { materialId, cantidad: cant }];
    setOrders((prev) => prev.map((o) => o.id === orden.id ? { ...o, materialesUsados: materialesActualizados } : o));
    supabase.from('ordenes').update({ materiales_usados: materialesActualizados }).eq('id', orden.supabaseId).then(({ error }) => { if (error) console.error(error); });
    postAsiento(fecha, `Consumo de material — orden ${orden.id} (asignado después de creada)`, '6135', cant * mat.costoUnitario, '1435', orden.centroCostoId);
  };
  ...
  const addOrderWithKardex = async (data) => {
    ...
    for (const mu of (data.materialesUsados || [])) {
      ...
      if (kNuevo) newEntries.push(nuevoKardexLocal(kNuevo, {
        materialId: mu.materialId, fecha: data.fecha, tipo: 'salida', cantidad: mu.cantidad,
    ...
    // Avisa a administración si algún material quedó en stock bajo (cruzó el umbral de 10) por esta orden.
    const bajaronDeUmbral = (data.materialesUsados || [])
      .map((mu) => matSnapshot.find((m) => m.id === mu.materialId))
      .filter((m) => m && m.stock < 10 && (m.stock + (data.materialesUsados.find((x) => x.materialId === m.id)?.cantidad || 0)) >= 10);
    if (bajaronDeUmbral.length > 0) {
      notificar(usuariosAdmin(), 'stock_bajo', 'Stock bajo',
        `${bajaronDeUmbral.map((m) => `${m.nombre} (quedan ${m.stock})`).join(', ')} — por la orden ${folio}.`, folio);
    }
    ...
    if (nuevoEstado === 'cancelada' && ordActual && ordActual.estado !== 'cancelada') {
      ...
      if (salidas.length > 0 && !yaDevueltas) {
        ...
        for (const s of salidas) {
          ...
          if (kNuevo) entradas.push(nuevoKardexLocal(kNuevo, {
            materialId: s.materialId, fecha: new Date().toISOString().slice(0, 10), tipo: 'entrada', cantidad: s.cantidad,
  ...
  const registerEntrada = async ({ materialId, cantidad, costoUnitario, fecha, referencia, centroCostoId }) => {
    ...
    if (kNuevo) setKardexLog((prev) => [...prev, nuevoKardexLocal(kNuevo, {
      materialId, fecha, tipo: 'entrada', cantidad: qty, costoUnitario: cu,
  ...
  const registerSalida = async ({ materialId, cantidad, fecha, referencia, centroCostoId, ordenSupabaseId }) => {
    ...
    if (kNuevo) setKardexLog((prev) => [...prev, nuevoKardexLocal(kNuevo, {
      materialId, fecha, tipo: 'salida', cantidad: qty, costoUnitario: m.costoUnitario,
  ...
  const addMaterialWithKardex = async (data) => {
    ...
    if (kNuevo) setKardexLog((prev) => [...prev, nuevoKardexLocal(kNuevo, {
      materialId: nuevo.id, fecha: kNuevo.fecha, tipo: 'entrada', cantidad: stock, costoUnitario: costo,
  ...
  const addFacturaManual = async (data) => {
    ...
    const folio = nextId('FM', facturasManuales, 5);
    ...
    setFacturasManuales((p) => [...p, { id: folio, supabaseId: nueva.id, ...data, monto, cuentaIngreso: cuentaIngresoCodigo, estadoPago: 'pendiente' }]);
    postAsiento(data.fecha, `Factura excepcional ${folio} — ${data.concepto}`, '1305', monto, cuentaIngresoCodigo, 'CC-ADM');
  };
  const toggleFacturaManualPago = (id) => {
    ...
    setFacturasManuales((prev) => prev.map((x) => {
      if (x.id !== id) return x;
      const hoy = new Date().toISOString().slice(0, 10);
      if (nuevo === 'pagada') postAsiento(hoy, `Cobro factura excepcional ${f.id} — ${x.concepto}`, '1110', x.monto, '1305', 'CC-ADM');
      else postAsiento(hoy, `Reversión cobro factura excepcional ${f.id} — ${x.concepto}`, '1305', x.monto, '1110', 'CC-ADM');
      return { ...x, estadoPago: nuevo };
    }));
    if (f.supabaseId) supabase.from('facturas_manuales').update({ estado_pago: nuevo }).eq('id', f.supabaseId).then(({ error }) => { if (error) console.error(error); });
  };
  ...
  const addVenta = async (data) => {
    ...
    if (faltantes.length > 0) { alert('Stock insuficiente:\n' + faltantes.join('\n')); return; }
    const folio = nextId('VT', ventas, 5);
    ...
    for (const l of data.lineas) {
      ...
      if (kNuevo) salidas.push(nuevoKardexLocal(kNuevo, {
        materialId: l.materialId, fecha: data.fecha, tipo: 'salida', cantidad: l.cantidad,
  ...
  const addEquipoComodato = async (data) => {
    ...
    setEquiposComodato((p) => [...p, { id: nextId('EQ', p, 5), supabaseId: nuevo.id, ...data, estado: 'en_almacen', clienteId: null, fechaAsignacion: null }]);
  };
  const asignarEquipoComodato = (id, clienteId, ordenId) => {
    if (bloqueadoPorLectura()) return;
    const eq = equiposComodato.find((e) => e.id === id);
    const fecha = new Date().toISOString().slice(0, 10);
    setEquiposComodato((prev) => prev.map((e) => e.id === id ? { ...e, estado: 'asignado', clienteId, ordenId: ordenId || null, fechaAsignacion: fecha } : e));
    if (eq?.supabaseId) supabase.from('equipos_comodato').update({ estado: 'asignado', cliente_id: clienteId, orden_id: ordenId || null, fecha_asignacion: fecha }).eq('id', eq.supabaseId).then(({ error }) => { if (error) console.error(error); });
  };
  const devolverEquipoComodato = (id) => {
    ...
    if (eq?.supabaseId) supabase.from('equipos_comodato').update({ estado: 'en_almacen', cliente_id: null, orden_id: null, fecha_devolucion: fecha }).eq('id', eq.supabaseId).then(({ error }) => { if (error) console.error(error); });
  };
  const bajaEquipoComodato = (id) => {
    ...
    pedirConfirmacion('¿Dar de baja este equipo (dañado / perdido)? Se conserva en el historial marcado como baja.', () => {
      ...
      if (eq?.supabaseId) supabase.from('equipos_comodato').update({ estado: 'baja' }).eq('id', eq.supabaseId).then(({ error }) => { if (error) console.error(error); });
    });
  };
  ...
  const addSquadra = async (data) => {
    ...
    setSquadras((p) => [...p, { id: nextId('SQ', p, 5), supabaseId: nueva.id, nombre: data.nombre, miembros: data.miembros, activa: true }]);
  };
  const toggleSquadraActiva = (id) => {
    if (bloqueadoPorLectura()) return;
    const sq = squadras.find((s) => s.id === id);
    const nuevaActiva = sq?.activa === false;
    setSquadras((prev) => prev.map((s) => s.id === id ? { ...s, activa: s.activa === false } : s));
    if (sq?.supabaseId) supabase.from('squadras').update({ activa: nuevaActiva }).eq('id', sq.supabaseId).then(({ error }) => { if (error) console.error(error); });
  };
  ...
  const addFlotaRegistro = async (tipo, data) => {
    ...
    if (!activo?.supabaseId) { alert('No se encontró el activo seleccionado.'); return; }
    const monto = Number(data.monto) || 0;
    const { data: nuevo, error } = await supabase.from('flota_log').insert({
      empresa_id: empresaActiva, tipo, activo_id: activo.supabaseId, fecha: data.fecha, monto,
    ...
    setFlotaLog((p) => [...p, { id: nextId('FL', p, 5), supabaseId: nuevo.id, tipo, ...data, activoId: activo.supabaseId, monto }]);
    if (monto > 0) {
    ...
    if (tipo === 'combustible' && data.kmActual) {
      ...
      supabase.from('activos_fijos').update({ km: Number(data.kmActual) }).eq('id', activo.supabaseId).then(({ error }) => { if (error) console.error(error); });
    }
  };
  ...
  const addMovimiento = async (data) => {
    ...
    setMovimientos((p) => [...p, { id: nextId('MV', p, 5), supabaseId: nuevo.id, ...data, monto }]);
  ...
  const deleteMovimiento = (id) => {
    ...
    pedirConfirmacion(`¿Eliminar este movimiento de ${m.tipo === 'gasto' ? 'gasto' : 'ingreso'} (${fmt(m.monto)})? Se revertirá también su registro contable.`, () => {
      ...
      if (m.supabaseId) supabase.from('movimientos').delete().eq('id', m.supabaseId).then(({ error }) => { if (error) console.error(error); });
    });
  };
  ...
  const exportReporte = () => {
    ...
    if (reportType === 'estado_resultados') {
      ...
      activos.forEach((a) => { const d = depreciacionInfo(a); const contab = depContabilizada(a.supabaseId); rows.push([a.nombre, a.categoria, centroCostoName(a.centroCostoId), a.costo, d.depAnual / 12, contab, a.costo - contab]); });
    ...
    if (data.tipo === 'inventario' && Array.isArray(data.lineas) && data.lineas.length > 0) {
      ...
      for (const linea of data.lineas) {
        ...
        const { data: kNuevo } = await supabase.from('kardex').insert({
          ...
          referencia: `Compra a crédito ${folio} — ${proveedorName(data.proveedorId)}`, saldo_cantidad: nuevoStock, saldo_valor: nuevoStock * nuevoCostoProm, centro_costo_id: null,
        }).select().single();
        if (kNuevo) nuevasEntradas.push(nuevoKardexLocal(kNuevo, {
          materialId: linea.materialId, fecha: data.fecha, tipo: 'entrada', cantidad, costoUnitario,
          referencia: `Compra a crédito ${folio} — ${proveedorName(data.proveedorId)}`, saldoCantidad: nuevoStock, saldoValor: nuevoStock * nuevoCostoProm, centroCostoId: 'CC-ADM',
        }));
      }
      setMaterials(matActualizados);
      setKardexLog((prev) => [...prev, ...nuevasEntradas]);
      postAsiento(data.fecha, `Compra de inventario a crédito ${folio} — ${data.concepto}`, '1435', monto, '2205', 'CC-ADM');
    } else {
      const cuentaGasto = gastoCategorias.find((c) => c.nombre === data.categoria)?.cuenta || '5195';
      postAsiento(data.fecha, `Compra a crédito ${folio} — ${data.concepto}`, cuentaGasto, monto, '2205', 'CC-ADM');
    }
    setCxp((p) => [...p, { id: folio, supabaseId: nuevo.id, estado: 'pendiente', ...data, monto }]);
  };
  const toggleCxpPago = (id) => {
    ...
    setCxp((prev) => prev.map((c) => {
      if (c.id !== id) return c;
      const hoy = new Date().toISOString().slice(0, 10);
      if (nuevo === 'pagada') postAsiento(hoy, `Pago a proveedor ${c.id} — ${c.concepto}`, '2205', c.monto, '1110', 'CC-ADM');
      else postAsiento(hoy, `Reversión pago a proveedor ${c.id} — ${c.concepto}`, '1110', c.monto, '2205', 'CC-ADM');
      return { ...c, estado: nuevo };
    }));
    if (cx?.supabaseId) supabase.from('cxp').update({ estado: nuevo }).eq('id', cx.supabaseId).then(({ error }) => { if (error) console.error(error); });
  };
  ...
  const addActivo = async (data) => {
    ...
    setActivos((p) => [...p, {
      id: nextId('AF', p, 5), supabaseId: nuevo.id, nombre: nuevo.nombre, categoria: nuevo.categoria, fechaCompra: nuevo.fecha_compra, costo,
      valorResidual, vidaUtilAnios, responsable: nuevo.responsable_id, centroCostoId: nuevo.centro_costo_id,
      placa: nuevo.placa, km: nuevo.km, seguroVencimiento: nuevo.seguro_vencimiento, revisionVencimiento: nuevo.revision_vencimiento, activo: true,
    }]);
    postAsiento(data.fechaCompra, `Compra de activo fijo — ${data.nombre}`, '1540', costo, '1110', data.centroCostoId || null);
  };
  const updateActivo = (id, campo, valor) => {
    const a = activos.find((x) => x.id === id);
    if (!a?.supabaseId) return;
    const valorFinal = ['costo', 'valorResidual', 'vidaUtilAnios', 'km'].includes(campo) ? Number(valor) || 0 : valor;
    setActivos((prev) => prev.map((x) => x.id === id ? { ...x, [campo]: valorFinal } : x));
    const columnaDb = { fechaCompra: 'fecha_compra', valorResidual: 'valor_residual', vidaUtilAnios: 'vida_util_anios', centroCostoId: 'centro_costo_id', seguroVencimiento: 'seguro_vencimiento', revisionVencimiento: 'revision_vencimiento' }[campo] || campo;
    supabase.from('activos_fijos').update({ [columnaDb]: valorFinal }).eq('id', a.supabaseId).then(({ error }) => { if (error) console.error('Error al editar activo:', error.message); });
  };
  const toggleActivoBaja = (id) => {
    const a = activos.find((x) => x.id === id);
    if (!a?.supabaseId) return;
    const nuevoValor = a.activo === false;
    pedirConfirmacion(nuevoValor ? '¿Reactivar este activo?' : '¿Dar de baja este activo? Deja de aparecer como vigente, pero conserva su historial de depreciación.', () => {
      setActivos((prev) => prev.map((x) => x.id === id ? { ...x, activo: nuevoValor } : x));
      supabase.from('activos_fijos').update({ activo: nuevoValor }).eq('id', a.supabaseId).then(({ error }) => { if (error) console.error(error); });
    });
  };
  const deleteActivo = (id) => {
    const a = activos.find((x) => x.id === id);
    if (!a) return;
    const tieneDepreciacion = depreciaciones.some((d) => d.activoId === id);
    if (tieneDepreciacion) { alert('Este activo ya tiene depreciación registrada — no se puede eliminar, pero puedes darlo de baja.'); return; }
    pedirConfirmacion(`¿Eliminar definitivamente el activo "${a.nombre}"? Esta acción no se puede deshacer.`, () => {
      setActivos((prev) => prev.filter((x) => x.id !== id));
      if (a.supabaseId) supabase.from('activos_fijos').delete().eq('id', a.supabaseId).then(({ error }) => { if (error) console.error(error); });
    });
  };
  ...
  const registrarDepreciacionMes = (periodo) => {
    activos.forEach(async (a) => {
      const yaRegistrado = depreciaciones.some((d) => d.activoId === a.supabaseId && d.periodo === periodo);
      if (yaRegistrado) return;
      const depreciable = a.costo - a.valorResidual;
      const depMensual = depreciable / a.vidaUtilAnios / 12;
      const acumuladaPrevia = depContabilizada(a.supabaseId);
      ...
      const { data: nueva } = await supabase.from('depreciaciones').insert({ empresa_id: empresaActiva, activo_id: a.supabaseId, periodo, fecha: `${periodo}-01`, monto }).select().single();
      if (nueva) setDepreciaciones((prev) => [...prev, { id: nueva.id, activoId: a.supabaseId, periodo, fecha: `${periodo}-01`, monto }]);
    });
  };
  ...
  const toggleMaterialActivo = (id) => {
    const m = materials.find((x) => x.id === id);
    const nuevo = m?.activo === false;
    const aplicar = () => {
      setMaterials((prev) => prev.map((x) => x.id === id ? { ...x, activo: nuevo } : x));
      supabase.from('materiales').update({ activo: nuevo }).eq('id', id).then(({ error }) => { if (error) console.error(error); });
    };
    if (nuevo) aplicar(); // reactivar no necesita confirmación
    else pedirConfirmacion(`¿Dar de baja "${m?.nombre}"? Deja de ofrecerse para nuevas órdenes, compras o ventas, pero conserva su historial.`, aplicar);
  };
  const updateMaterial = (id, campo, valor) => {
    setMaterials((prev) => prev.map((m) => m.id === id ? { ...m, [campo]: valor } : m));
    supabase.from('materiales').update({ [campo === 'costoUnitario' ? 'costo_unitario' : campo]: valor }).eq('id', id).then(({ error }) => { if (error) console.error(error); });
  };
  ...
  const addAnticipo = async (data) => {
    ...
    setAnticipos((p) => [...p, { id: nextId('AN', p, 5), supabaseId: nuevo.id, ...data, monto, estado: 'pendiente' }]);
  ...
  const addPrestamo = async (data) => {
    ...
    setPrestamos((p) => [...p, { id: nextId('PR', p, 5), supabaseId: nuevo.id, empleadoId: data.empleadoId, fecha: data.fecha, motivo: data.motivo, montoTotal, cuotas, montoCuota, saldoPendiente: montoTotal, cuotasPagadas: 0, estado: 'activo' }]);
  ...
  const addNomina = async (data) => {
    ...
    const folio = nextId('NM', nomina, 5);
    setNomina((p) => [...p, { id: folio, supabaseId: nueva.id, estado: 'pendiente', ...data, montoBruto: bruto, descuentoAnticipo, descuentoPrestamo, monto: neto }]);
    // Técnicos → Costo de mano de obra (6140); administrativos → Gasto de nómina (5105)
    const cuentaGasto = emp?.rol === 'Técnico' ? '6140' : '5105';
    postAsiento(data.fechaPago, `Nómina ${folio} · ${data.periodo} — ${empleadoName(data.empleadoId)}`, cuentaGasto, bruto, '2510', 'CC-ADM');
    // Liquidación de anticipos y préstamo contra la nómina por pagar (ya no se pagan dos veces)
    if (descuentoAnticipo > 0) {
      postAsiento(data.fechaPago, `Descuento de anticipo en nómina ${folio} — ${empleadoName(data.empleadoId)}`, '2510', descuentoAnticipo, '1310', 'CC-ADM');
      setAnticipos((prev) => prev.map((a) => a.empleadoId === data.empleadoId && a.estado === 'pendiente' ? { ...a, estado: 'descontado' } : a));
      const idsAnticipos = anticiposPend.map((a) => a.supabaseId).filter(Boolean);
    ...
    if (descuentoPrestamo > 0 && prestamoActivo) {
      postAsiento(data.fechaPago, `Descuento de cuota de préstamo en nómina ${folio} — ${empleadoName(data.empleadoId)}`, '2510', descuentoPrestamo, '1315', 'CC-ADM');
      ...
      if (prestamoActivo.supabaseId) supabase.from('prestamos').update({ saldo_pendiente: nuevoSaldo, cuotas_pagadas: prestamoActivo.cuotasPagadas + 1, estado: nuevoEstadoPrestamo }).eq('id', prestamoActivo.supabaseId)
        .then(({ error }) => { if (error) console.error(error); });
    }
  };
  ...
  const toggleNominaPago = (id) => {
    ...
    setNomina((prev) => prev.map((n) => {
      ...
      const hoy = new Date().toISOString().slice(0, 10);
      if (nuevo === 'pagado') {
        postAsiento(hoy, `Pago de nómina ${n.id} — ${empleadoName(n.empleadoId)} (${n.periodo})`, '2510', n.monto, '1110', 'CC-ADM');
      } else {
        // Reversión: asiento inverso para no dejar el pago colgado en la contabilidad
        postAsiento(hoy, `Reversión pago de nómina ${n.id} — ${empleadoName(n.empleadoId)} (${n.periodo})`, '1110', n.monto, '2510', 'CC-ADM');
      }
      return { ...n, estado: nuevo };
    }));
    if (nom?.supabaseId) supabase.from('nomina').update({ estado: nuevo }).eq('id', nom.supabaseId).then(({ error }) => { if (error) console.error(error); });
  };
  ...
  const addAsiento = (data) => {
    ...
    const nuevo = { id: nextId('DC', asientosRef.current, 5), ...data, montoDebe: Number(data.montoDebe) || 0, montoHaber: Number(data.montoHaber) || 0 };
  ...
  const deleteCuenta = (codigo) => {
    ...
    pedirConfirmacion(`¿Eliminar la cuenta ${codigo} — ${c?.nombre}? Esta acción no se puede deshacer.`, () => {
      setCuentas((prev) => prev.filter((x) => x.codigo !== codigo));
      if (c?.id) supabase.from('cuentas').delete().eq('id', c.id).then(({ error }) => { if (error) console.error(error); });
    });
  };
  ...
  const deleteCentro = (id) => {
    ...
    pedirConfirmacion(`¿Eliminar el centro de costo "${c?.nombre}"? Esta acción no se puede deshacer.`, () => {
      setCentros((prev) => prev.filter((x) => x.id !== id));
  ...
  const deleteCategoria = (tipo, nombre) => {
    ...
    pedirConfirmacion(`¿Eliminar la categoría "${nombre}"? Esta acción no se puede deshacer.`, () => {
      const setter = tipo === 'gasto' ? setGastoCategorias : setIngresoCategorias;
  ...
  const clientesFiltrados = clients.filter((c) => !clientesBusqueda.trim() || c.nombre.toLowerCase().includes(clientesBusqueda.trim().toLowerCase()) || (c.telefono || '').includes(clientesBusqueda.trim()) || (c.direccion || '').toLowerCase().includes(clientesBusqueda.trim().toLowerCase()));
  const clientesPag = usePaginado(clientesFiltrados, 12);
  const empleadosFiltrados = empleados.filter((e) => !empleadosBusqueda.trim() || e.nombre.toLowerCase().includes(empleadosBusqueda.trim().toLowerCase()) || (e.rol || '').toLowerCase().includes(empleadosBusqueda.trim().toLowerCase()));
  const empleadosPag = usePaginado(empleadosFiltrados, 12);
  const proveedoresFiltrados = proveedores.filter((p) => !proveedoresBusqueda.trim() || p.nombre.toLowerCase().includes(proveedoresBusqueda.trim().toLowerCase()) || (p.contacto || '').toLowerCase().includes(proveedoresBusqueda.trim().toLowerCase()));
  const proveedoresPag = usePaginado(proveedoresFiltrados, 12);
  const materialesFiltrados = materials.filter((m) => !materialesBusqueda.trim() || m.nombre.toLowerCase().includes(materialesBusqueda.trim().toLowerCase()))
    .sort((a, b) => {
      if (!materialesOrden.campo) return 0;
      const va = a[materialesOrden.campo], vb = b[materialesOrden.campo];
      return (typeof va === 'string' ? va.localeCompare(vb) : va - vb) * materialesOrden.dir;
    });
  const materialesPag = usePaginado(materialesFiltrados, 15);
  const ventasFiltradas = ventasOrdenadas.filter((v) => !ventasBusqueda.trim() || v.id.toLowerCase().includes(ventasBusqueda.trim().toLowerCase()) || clientName(v.clienteId).toLowerCase().includes(ventasBusqueda.trim().toLowerCase()));
  const ventasPagFiltrada = usePaginado(ventasFiltradas, 15);
  const cxpOrdenadas = [...cxp].sort((a, b) => a.vencimiento.localeCompare(b.vencimiento));
  const cxpPag = usePaginado(cxpOrdenadas, 15);
  const cxpFiltradas = cxpOrdenadas.filter((c) => !cxpBusqueda.trim() || c.concepto.toLowerCase().includes(cxpBusqueda.trim().toLowerCase()) || proveedorName(c.proveedorId).toLowerCase().includes(cxpBusqueda.trim().toLowerCase()))
    .sort((a, b) => {
      if (!cxpOrden.campo) return 0;
      if (cxpOrden.campo === 'proveedor') return proveedorName(a.proveedorId).localeCompare(proveedorName(b.proveedorId)) * cxpOrden.dir;
      const va = a[cxpOrden.campo], vb = b[cxpOrden.campo];
      return (typeof va === 'string' ? va.localeCompare(vb) : va - vb) * cxpOrden.dir;
    });
  const cxpPagFiltrada = usePaginado(cxpFiltradas, 15);
  const nominaPag = usePaginado(nomina, 15);
  const facturasManualesPag = usePaginado(facturasManuales, 15);
  const anticiposPag = usePaginado(anticipos, 15);
  const prestamosPag = usePaginado(prestamos, 15);
  const projectsPag = usePaginado(projects, 12);
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif', backgroundColor: C.bg, minHeight: '100vh', display: 'flex' }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media print {
          ...
          aside, .no-print, nav, button, select, input, .print-hide { display: none !important; }
      ...
      <div className="flex-1 p-4 md:p-8" style={{ maxWidth: 1220, minWidth: 0, overflowX: 'hidden' }}>
        <div className="print-only" style={{ alignItems: 'center', gap: 10, marginBottom: 16, paddingBottom: 10, borderBottom: `2px solid ${C.navy}` }}>
          <svg width="30" height="30" viewBox="0 0 24 24" fill="none"><path d="M4 12c0-4.4 3.6-8 8-8M4 12c0 4.4 3.6 8 8 8M4 12h16M12 4c2.5 2 4 5 4 8s-1.5 6-4 8" stroke={C.navy} strokeWidth="1.6" strokeLinecap="round" /></svg>
          <div>
            <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 15, color: C.navy }}>{(empresaActual?.nombre || 'GESTIONA ERP').toUpperCase()}</div>
            <div style={{ fontSize: 10.5, color: C.muted }}>Impreso el {new Date().toLocaleDateString('es-MX', { day: '2-digit', month: 'long', year: 'numeric' })}</div>
          </div>
        </div>
        {cargandoDatos && (
          <div className="no-print flex items-center gap-2" style={{ backgroundColor: '#F0FBF9', border: `1px solid ${C.cyanDark}`, borderRadius: 10, padding: '8px 14px', marginBottom: 14, fontSize: 12.5, color: C.cyanDark, fontWeight: 600 }}>
            <span style={{ width: 14, height: 14, border: `2px solid ${C.cyanDark}`, borderTopColor: 'transparent', borderRadius: '50%', display: 'inline-block', animation: 'girar 0.7s linear infinite' }} />
        ...
        {proveedorSel && (() => {
          const p = proveedores.find((x) => x.id === proveedorSel);
          if (!p) return null;
          const historial = cxp.filter((c) => c.proveedorId === proveedorSel).sort((a, b) => b.fecha.localeCompare(a.fecha));
          const totalComprado = historial.reduce((s, c) => s + Number(c.monto), 0);
          const totalPendiente = historial.filter((c) => c.estado !== 'pagada').reduce((s, c) => s + Number(c.monto), 0);
          return (
            <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(11,22,34,0.5)', zIndex: 65, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setProveedorSel(null)}>
              <div onClick={(e) => e.stopPropagation()} className="rounded-xl p-4" style={{ backgroundColor: '#fff', width: 420, maxWidth: '95vw', maxHeight: '85vh', overflow: 'auto', boxShadow: '0 20px 50px rgba(0,0,0,0.25)' }}>
                <div className="flex items-center justify-between mb-1">
                  <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 15, color: C.slate }}>{p.nombre}</div>
                  <button onClick={() => setProveedorSel(null)}><X size={18} color={C.muted} /></button>
                </div>
                {p.contacto && <Row label="Contacto">{p.contacto}</Row>}
                {p.telefono && <Row label="Teléfono">{p.telefono}</Row>}
                <div className="flex gap-4 mt-2 mb-3">
                  <div><div style={{ fontSize: 10.5, color: C.muted, textTransform: 'uppercase' }}>Comprado</div><div style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, color: C.slate }}>{fmt(totalComprado)}</div></div>
                  {totalPendiente > 0 && <div><div style={{ fontSize: 10.5, color: C.muted, textTransform: 'uppercase' }}>Pendiente</div><div style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 700, color: C.amber }}>{fmt(totalPendiente)}</div></div>}
                </div>
                <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase', marginBottom: 6 }}>Historial de compras</div>
                {historial.length === 0 ? <div style={{ fontSize: 12.5, color: C.muted }}>Sin compras registradas todavía.</div> : (
                  <div className="rounded-lg overflow-hidden" style={{ border: `1px solid ${C.border}` }}>
                    {historial.map((c, i) => (
                      <div key={c.id} onClick={() => { setProveedorSel(null); abrirTrazabilidad(c.id); }}
                        className="flex justify-between items-center" style={{ padding: '8px 10px', borderTop: i > 0 ? `1px solid ${C.border}` : 'none', cursor: 'pointer' }}>
                        <div>
                          <div style={{ fontSize: 12.5, color: C.slate }}>{c.concepto}</div>
                          <div style={{ fontSize: 10.5, color: C.muted }}>{c.id} · {c.fecha}</div>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12.5, color: C.slate }}>{fmt(c.monto)}</div>
                          <div style={{ fontSize: 10, color: c.estado === 'pagada' ? C.cyanDark : C.amber }}>{c.estado === 'pagada' ? 'Pagada' : 'Pendiente'}</div>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {buscadorGlobalAbierto && (() => {
          const q = buscadorGlobalTexto.trim().toLowerCase();
          const resOrdenes = q.length < 2 ? [] : orders.filter((o) => o.id.toLowerCase().includes(q) || clientName(o.clienteId).toLowerCase().includes(q) || (o.direccion || '').toLowerCase().includes(q)).slice(0, 5);
          const resClientes = q.length < 2 ? [] : clients.filter((c) => c.nombre.toLowerCase().includes(q)).slice(0, 5);
          const resEmpleados = q.length < 2 ? [] : empleados.filter((e) => e.nombre.toLowerCase().includes(q)).slice(0, 5);
          const resProveedores = q.length < 2 ? [] : proveedores.filter((p) => p.nombre.toLowerCase().includes(q)).slice(0, 5);
          const sinResultados = q.length >= 2 && resOrdenes.length + resClientes.length + resEmpleados.length + resProveedores.length === 0;
          const ir = (tabDestino, callback) => { setTab(tabDestino); setBuscadorGlobalAbierto(false); setBuscadorGlobalTexto(''); if (callback) callback(); };
          return (
            <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(11,22,34,0.5)', zIndex: 70, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '10vh 16px' }} onClick={() => setBuscadorGlobalAbierto(false)}>
              <div onClick={(e) => e.stopPropagation()} className="rounded-xl" style={{ backgroundColor: '#fff', width: 480, maxWidth: '100%', maxHeight: '70vh', overflow: 'auto', boxShadow: '0 20px 50px rgba(0,0,0,0.25)' }}>
                <div style={{ padding: 14, borderBottom: `1px solid ${C.border}` }}>
                  <input autoFocus value={buscadorGlobalTexto} onChange={(e) => setBuscadorGlobalTexto(e.target.value)} placeholder="Buscar una orden, cliente, empleado o proveedor..." style={{ ...inputStyle, width: '100%', fontSize: 14 }} />
                </div>
                <div style={{ padding: 8 }}>
                  {q.length < 2 && <div style={{ padding: 16, color: C.muted, fontSize: 12.5, textAlign: 'center' }}>Escribe al menos 2 letras para buscar.</div>}
                  {sinResultados && <div style={{ padding: 16, color: C.muted, fontSize: 12.5, textAlign: 'center' }}>Sin resultados para "{buscadorGlobalTexto}".</div>}
                  {resOrdenes.length > 0 && (
                    <div className="mb-2">
                      <div style={{ fontSize: 10.5, fontWeight: 700, color: C.muted, textTransform: 'uppercase', padding: '6px 10px' }}>Órdenes de trabajo</div>
                      {resOrdenes.map((o) => (
                        <div key={o.id} onClick={() => ir('ordenes', () => { setAgendaSel(o.id); setAgendaEdit(false); })} style={{ padding: '8px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 13 }} className="hover:bg-gray-50">
                          <span style={{ fontFamily: 'JetBrains Mono, monospace', color: C.cyanDark }}>{o.id}</span> — {clientName(o.clienteId)} · {o.direccion || o.tipoServicio}
                        </div>
                      ))}
                    </div>
                  )}
                  {resClientes.length > 0 && (
                    <div className="mb-2">
                      <div style={{ fontSize: 10.5, fontWeight: 700, color: C.muted, textTransform: 'uppercase', padding: '6px 10px' }}>Clientes</div>
                      {resClientes.map((c) => (
                        <div key={c.id} onClick={() => ir('clientes')} style={{ padding: '8px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>{c.nombre} <span style={{ color: C.muted, fontSize: 11.5 }}>{c.telefono}</span></div>
                      ))}
                    </div>
                  )}
                  {resEmpleados.length > 0 && (
                    <div className="mb-2">
                      <div style={{ fontSize: 10.5, fontWeight: 700, color: C.muted, textTransform: 'uppercase', padding: '6px 10px' }}>Empleados</div>
                      {resEmpleados.map((e) => (
                        <div key={e.id} onClick={() => ir('rh')} style={{ padding: '8px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>{e.nombre} <span style={{ color: C.muted, fontSize: 11.5 }}>{e.rol}</span></div>
                      ))}
                    </div>
                  )}
                  {resProveedores.length > 0 && (
                    <div className="mb-2">
                      <div style={{ fontSize: 10.5, fontWeight: 700, color: C.muted, textTransform: 'uppercase', padding: '6px 10px' }}>Proveedores</div>
                      {resProveedores.map((p) => (
                        <div key={p.id} onClick={() => ir('proveedores')} style={{ padding: '8px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>{p.nombre}</div>
                      ))}
                    </div>
                  )}
                </div>
        ...
        {trazaActual && (() => {
          const puedeAtras = trazaStack.length > 1;
          const encabezado = (titulo) => (
            <div className="flex items-center justify-between mb-1">
              <div className="flex items-center gap-2">
                {puedeAtras && <button onClick={trazaAtras} title="Atrás" style={{ color: C.cyanDark }}>←</button>}
                <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 15, color: C.slate }}>{titulo}</div>
              </div>
              <button onClick={cerrarTraza}><X size={18} color={C.muted} /></button>
            </div>
          );

          // ---------- Modo "flujo": el flujograma con cajas y líneas punteadas ----------
          if (trazaActual.modo === 'flujo') {
            const { tipo: tipoRaiz, raiz, asientos: aRel, kardex: kRel } = trazaDeOrden(trazaActual.folio);
            const etiquetaRaiz = { orden: 'Orden', venta: 'Venta', cxp: 'Cuenta por pagar', nomina: 'Nómina', factura: 'Factura' }[tipoRaiz] || 'Documento';
            const nodos = [
              ...aRel.map((a) => ({ tipo: 'asiento', color: '#8E6FCB', titulo: a.id, sub: a.glosa.length > 26 ? a.glosa.slice(0, 26) + '…' : a.glosa, doc: a })),
              ...kRel.map((k) => ({ tipo: 'kardex', color: C.amber, titulo: k.id, sub: `${k.tipo === 'entrada' ? 'Entrada' : 'Salida'} ${k.cantidad}`, doc: k })),
            ];
            const boxW = 150, boxH = 52, gapY = 20, marginY = 20;
            const totalH = Math.max(120, nodos.length * (boxH + gapY) + marginY);
            const rootX = 20, rootY = totalH / 2 - boxH / 2;
            const branchX = 280;
            const svgW = branchX + boxW + 20;
            return (
              <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(11,22,34,0.5)', zIndex: 65, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={cerrarTraza}>
                <div onClick={(e) => e.stopPropagation()} className="rounded-xl p-4" style={{ backgroundColor: '#fff', maxWidth: '95vw', maxHeight: '85vh', overflow: 'auto', boxShadow: '0 20px 50px rgba(0,0,0,0.25)' }}>
                  {encabezado(`Trazabilidad — ${etiquetaRaiz} ${trazaActual.folio}`)}
                  <div style={{ fontSize: 11.5, color: C.muted, marginBottom: 10 }}>Toca cualquier caja para ver ese documento sin salir de aquí.</div>
                  {nodos.length === 0 ? (
                    <div style={{ padding: '30px 20px', color: C.muted, fontSize: 12.5, textAlign: 'center' }}>Aún no hay comprobantes ni movimientos ligados a este documento.</div>
                  ) : (
                    <svg width={svgW} height={totalH} viewBox={`0 0 ${svgW} ${totalH}`} style={{ display: 'block' }}>
                      {nodos.map((n, i) => {
                        const y = marginY / 2 + i * (boxH + gapY);
                        const startX = rootX + boxW, startY = rootY + boxH / 2;
                        const endY = y + boxH / 2;
                        return <path key={'p' + i} d={`M ${startX} ${startY} C ${startX + 40} ${startY}, ${branchX - 40} ${endY}, ${branchX} ${endY}`} fill="none" stroke="#C7CDD4" strokeWidth="1.5" strokeDasharray="4 3" />;
                      })}
                      <g onClick={() => raiz && verDocumentoEnTraza(tipoRaiz, raiz)} style={{ cursor: raiz ? 'pointer' : 'default' }}>
                        <rect x={rootX} y={rootY} width={boxW} height={boxH} rx="8" fill="#F0FBF9" stroke={C.cyanDark} strokeWidth="1" />
                        <text x={rootX + boxW / 2} y={rootY + 21} textAnchor="middle" style={{ fontSize: 12, fontWeight: 700, fill: C.slate }}>{trazaActual.folio}</text>
                        <text x={rootX + boxW / 2} y={rootY + 37} textAnchor="middle" style={{ fontSize: 10, fill: C.muted }}>{etiquetaRaiz}</text>
                      </g>
                      {nodos.map((n, i) => {
                        const y = marginY / 2 + i * (boxH + gapY);
                        return (
                          <g key={'n' + i} onClick={() => verDocumentoEnTraza(n.tipo, n.doc)} style={{ cursor: 'pointer' }}>
                            <rect x={branchX} y={y} width={boxW} height={boxH} rx="8" fill="#fff" stroke={n.color} strokeWidth="1" />
                            <text x={branchX + boxW / 2} y={y + 21} textAnchor="middle" style={{ fontSize: 11.5, fontWeight: 700, fill: C.slate }}>{n.titulo}</text>
                            <text x={branchX + boxW / 2} y={y + 37} textAnchor="middle" style={{ fontSize: 9.5, fill: C.muted }}>{n.sub}</text>
                          </g>
                        );
                      })}
                    </svg>
          ...
          // ---------- Modo "orden": ficha completa de la orden (fotos, materiales, estado) sin salir de la pantalla ----------
          if (trazaActual.modo === 'orden') {
            const o = trazaActual.doc;
            return (
              <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(11,22,34,0.5)', zIndex: 65, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={cerrarTraza}>
                <div onClick={(e) => e.stopPropagation()} className="rounded-xl p-4" style={{ backgroundColor: '#fff', width: 420, maxWidth: '95vw', maxHeight: '85vh', overflow: 'auto', boxShadow: '0 20px 50px rgba(0,0,0,0.25)' }}>
                  {encabezado(`Orden ${o.id}`)}
                  <Row label="Estado">{ESTADOS[o.estado]?.label || o.estado}</Row>
                  <Row label="Cliente">{clientName(o.clienteId)}</Row>
                  <Row label="Técnico">{techName(o.tecnicoId)}</Row>
                  <Row label="Tipo de servicio">{o.tipoServicio}</Row>
                  <Row label="Dirección">{o.direccion || '—'}</Row>
                  <Row label="Fecha y hora">{o.fecha}{o.hora ? ` · ${o.hora}` : ''}</Row>
                  {!ocultaValores('ordenes') && <Row label="Precio al cliente">{fmt(o.precioCliente)}</Row>}
                  {!ocultaValores('ordenes') && <Row label="Mano de obra">{fmt(o.costoManoObra)}</Row>}
                  {o.notas && <Row label="Notas">{o.notas}</Row>}
                  <div style={{ marginTop: 10 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase', marginBottom: 4 }}>Materiales asignados</div>
                    {(o.materialesUsados || []).length === 0 ? <div style={{ fontSize: 12, color: C.muted }}>Sin materiales asignados.</div>
                      : (o.materialesUsados || []).map((mu, mi) => { const mat = materials.find((m) => m.id === mu.materialId); return <div key={mi} style={{ fontSize: 12, color: C.slate }}>{mu.cantidad}× {mat?.nombre || mu.materialId}</div>; })}
                  </div>
                  <div style={{ marginTop: 10 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase', marginBottom: 4 }}>Fotos</div>
                    {(o.fotos || []).length === 0 ? <div style={{ fontSize: 12, color: C.muted }}>Sin fotos.</div> : (
                      <div className="flex flex-wrap gap-2">
                        {(o.fotos || []).map((foto, fi) => (
                          <img key={fi} src={foto.url} alt="Foto de la orden" style={{ width: 64, height: 64, objectFit: 'cover', borderRadius: 8, border: `1px solid ${C.border}`, cursor: 'pointer' }} onClick={() => window.open(foto.url, '_blank')} />
                        ))}
                      </div>
                    )}
                  </div>
                  <button onClick={() => setAgendaSel(o.id)} className="w-full mt-4 py-2 rounded-lg text-sm font-semibold" style={{ backgroundColor: C.navy, color: '#fff' }}>Abrir para editar</button>
                </div>
              </div>
            );
          }

          // ---------- Modo "detalle": tarjeta simple de un asiento o movimiento de Kardex ----------
          const { tipo, doc, folioRelacionado } = trazaActual;
          const titulo = tipo === 'asiento' ? `Comprobante ${doc.id}` : `Movimiento ${doc.id}`;
          return (
            <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(11,22,34,0.5)', zIndex: 65, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={cerrarTraza}>
              <div onClick={(e) => e.stopPropagation()} className="rounded-xl p-4" style={{ backgroundColor: '#fff', width: 360, maxWidth: '95vw', boxShadow: '0 20px 50px rgba(0,0,0,0.25)' }}>
                {encabezado(titulo)}
                {tipo === 'asiento' ? (
                  <>
                    <Row label="Fecha">{doc.fecha}</Row>
                    <Row label="Concepto">{doc.glosa}</Row>
                    <Row label="Cuenta debe">{doc.cuentaDebe} — {fmt(doc.montoDebe)}</Row>
                    <Row label="Cuenta haber">{doc.cuentaHaber} — {fmt(doc.montoHaber)}</Row>
                  </>
                ) : (
                  <>
                    <Row label="Fecha">{doc.fecha}</Row>
                    <Row label="Material">{materials.find((m) => m.id === doc.materialId)?.nombre || doc.materialId}</Row>
                    <Row label="Movimiento">{doc.tipo === 'entrada' ? 'Entrada' : 'Salida'} — {doc.cantidad}</Row>
                    <Row label="Referencia">{doc.referencia}</Row>
                  </>
                )}
                {folioRelacionado && (
                  <button onClick={() => trazaIrA({ modo: 'flujo', folio: folioRelacionado })} className="w-full mt-4 py-2 rounded-lg text-sm font-semibold" style={{ backgroundColor: C.cyanDark, color: '#fff' }}>
                    Ver flujo de {folioRelacionado} →
                  </button>
                )}
              </div>
            </div>
                      ...
                      {!agendaEdit ? (
                        <div>
                          ...
                          <div style={{ marginTop: 12 }}>
                            <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase', marginBottom: 6 }}>Materiales asignados</div>
                            {(o.materialesUsados || []).length === 0 ? (
                              <div style={{ fontSize: 12, color: C.muted }}>Sin materiales asignados todavía.</div>
                            ) : (o.materialesUsados || []).map((mu, mi) => {
                              const mat = materials.find((m) => m.id === mu.materialId);
                              return <div key={mi} style={{ fontSize: 12, color: C.slate, marginBottom: 2 }}>{mu.cantidad}× {mat?.nombre || mu.materialId}</div>;
                            })}
                            {tieneAcceso(sesion.rol, 'ordenes') && !esSoloLectura(sesion.rol, 'ordenes') && (
                              <div className="flex gap-2 items-end mt-2">
                                <select value={agendaMaterialSel} onChange={(e) => setAgendaMaterialSel(e.target.value)} style={{ ...inputStyle, fontSize: 12, padding: '5px 6px', flex: 1 }}>
                                  <option value="">Elegir material…</option>
                                  {materials.filter((m) => m.activo !== false).map((m) => <option key={m.id} value={m.id}>{m.nombre} (stock {m.stock})</option>)}
                                </select>
                                <input type="number" value={agendaMaterialCant} onChange={(e) => setAgendaMaterialCant(e.target.value)} placeholder="Cant." style={{ ...inputStyle, fontSize: 12, padding: '5px 6px', width: 60 }} />
                                <button onClick={() => { if (agendaMaterialSel && agendaMaterialCant) { asignarMaterialAOrden(o, agendaMaterialSel, agendaMaterialCant); setAgendaMaterialSel(''); setAgendaMaterialCant(''); } }}
                                  className="px-2.5 py-1.5 rounded-lg text-xs font-semibold" style={{ backgroundColor: C.navy, color: '#fff' }}>+ Asignar</button>
                              </div>
                            )}
                          </div>
                          <div style={{ marginTop: 12 }}>
                            <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase', marginBottom: 6 }}>Fotos</div>
                            <div className="flex flex-wrap gap-2">
                              {(o.fotos || []).map((foto, fi) => (
                                <div key={fi} style={{ position: 'relative', width: 70, height: 70 }}>
                                  <img src={foto.url} alt="Foto de la orden" style={{ width: 70, height: 70, objectFit: 'cover', borderRadius: 8, border: `1px solid ${C.border}`, cursor: 'pointer' }}
                                    onClick={() => window.open(foto.url, '_blank')} />
                                  <button onClick={() => eliminarFotoOrden(o, foto)} style={{ position: 'absolute', top: -6, right: -6, backgroundColor: C.coral, borderRadius: '50%', width: 18, height: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <X size={11} color="#fff" />
                                  </button>
                                </div>
                              ))}
                              <label style={{ width: 70, height: 70, borderRadius: 8, border: `1px dashed ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: C.muted, fontSize: 22 }}>
                                +
                                <input type="file" accept="image/*" capture="environment" style={{ display: 'none' }}
                                  onChange={(e) => { if (e.target.files[0]) subirFotoOrden(o, e.target.files[0]); e.target.value = ''; }} />
                              </label>
                            </div>
                          </div>
        ...
        <div className="flex justify-end items-center gap-2.5 mb-4 no-print" style={{ position: 'relative' }}>
          <button onClick={() => setBuscadorGlobalAbierto(true)} className="flex items-center justify-center rounded-xl" style={{ width: 42, height: 42, backgroundColor: C.card, border: `1px solid ${C.border}` }} title="Buscar en todo el sistema">
        ...
        {tab === 'ordenes' && (
          <div>
            ...
            {showOrderForm && (
              <OrderForm clients={clients.filter((c) => c.activo !== false)} techs={techs} materials={materials.filter((m) => m.activo !== false)} projects={projects} centros={centros} catalogoPartidas={catalogoPartidas} tiposServicio={tiposServicio}
                onSubmit={(data) => { addOrderWithKardex(data); setShowOrderForm(false); }} />
            )}

            <div className="flex items-center gap-1.5 mt-5" style={{ position: 'relative', maxWidth: 380 }}>
              <input value={ordersSearch} onChange={(e) => setOrdersSearch(e.target.value)} placeholder="Buscar por folio, cliente, técnico o ubicación..."
                style={{ ...inputStyle, width: '100%', paddingLeft: 32, fontSize: 13.5 }} />
              <span style={{ position: 'absolute', left: 10, color: C.muted, fontSize: 15, pointerEvents: 'none' }}>🔍</span>
              {ordersSearch && (
                <button onClick={() => setOrdersSearch('')} style={{ position: 'absolute', right: 10, color: C.muted }}><X size={14} /></button>
              )}
            </div>
            <div className="flex gap-1.5 mt-3 mb-3 items-center flex-wrap">
              ...
              <table>
                ...
                <tbody>
                  {ordersPag.paginaActual.map((o) => (
                    <tr key={o.id}>
                      <td onClick={() => { setAgendaSel(o.id); setAgendaEdit(false); }} style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, cursor: 'pointer', color: C.cyanDark, textDecoration: 'underline' }} title="Ver detalle completo">{o.id}{o.referencia && <div style={{ fontSize: 10, color: C.muted, fontFamily: 'Inter, sans-serif', textDecoration: 'none' }}>{o.referencia}</div>}</td>
                      ...
                      <td>
                        <div className="flex gap-2">
                          <button onClick={() => abrirTrazabilidad(o.id)} title="Ver trazabilidad"><GitBranch size={14} color={C.cyanDark} /></button>
        ...
        {tab === 'proyectos' && (
          <div>
            ...
            {showProjectForm && (
              <div className="rounded-xl p-4 mt-4 flex flex-wrap gap-3 items-end" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
                <ProjectFormFields clients={clients} onSubmit={async (data) => {
                  ...
                  setProjects((p) => [...p, { id: nextId('PY', p, 5), supabaseId: nuevo.id, estado: 'activo', ...data }]);
            ...
            <div className="grid grid-cols-2 gap-3 mt-5">
              {projectsPag.paginaActual.map((p) => {
                ...
                return (
                  <div key={p.id} className="rounded-xl p-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
                    <div className="flex items-center justify-between">
                      ...
                      <select value={p.estado} onChange={(e) => { setProjects((prev) => prev.map((x) => x.id === p.id ? { ...x, estado: e.target.value } : x)); if (p.supabaseId) supabase.from('proyectos').update({ estado: e.target.value }).eq('id', p.supabaseId).then(({ error }) => { if (error) console.error(error); }); }}
        ...
        {tab === 'clientes' && (
          <div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => descargarCSVInforme('clientes', [['Nombre', 'Teléfono', 'Dirección', 'Estado'], ...clientesFiltrados.map((c) => [c.nombre, c.telefono || '', c.direccion || '', c.activo === false ? 'Inactivo' : 'Activo'])])}
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-semibold" style={{ backgroundColor: '#fff', color: C.slate, border: `1px solid ${C.border}` }}>
            ...
            <div className="flex items-center gap-1.5 mt-5" style={{ position: 'relative', maxWidth: 380 }}>
              <input value={clientesBusqueda} onChange={(e) => setClientesBusqueda(e.target.value)} placeholder="Buscar por nombre, teléfono o dirección..." style={{ ...inputStyle, width: '100%', paddingLeft: 32, fontSize: 13.5 }} />
              <span style={{ position: 'absolute', left: 10, color: C.muted, fontSize: 15, pointerEvents: 'none' }}>🔍</span>
              {clientesBusqueda && <button onClick={() => setClientesBusqueda('')} style={{ position: 'absolute', right: 10, color: C.muted }}><X size={14} /></button>}
            </div>
            <div className="grid grid-cols-2 gap-3 mt-4">
              {clientesPag.paginaActual.map((c) => {
              ...
              {clientesFiltrados.length === 0 && <div style={{ gridColumn: '1 / -1', color: C.muted, textAlign: 'center', padding: 24 }}>Sin clientes que coincidan con la búsqueda.</div>}
            </div>
            <Paginador p={clientesPag} />
          </div>
        )}
        ...
        {tab === 'inventario' && (
          <div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={() => descargarCSVInforme('materiales', [['Material', 'Unidad', 'Costo promedio', 'Stock', 'Valor en inventario', 'Estado'], ...materialesFiltrados.map((m) => [m.nombre, m.unidad, m.costoUnitario, m.stock, m.stock * m.costoUnitario, m.activo === false ? 'De baja' : (m.stock < 10 ? 'Stock bajo' : 'Suficiente')])])}
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-semibold" style={{ backgroundColor: '#fff', color: C.slate, border: `1px solid ${C.border}` }}>
                  <Download size={15} /> Exportar CSV
                </button>
            ...
            <div className="flex items-center gap-1.5 mt-5" style={{ position: 'relative', maxWidth: 380 }}>
              <input value={materialesBusqueda} onChange={(e) => setMaterialesBusqueda(e.target.value)} placeholder="Buscar material..." style={{ ...inputStyle, width: '100%', paddingLeft: 32, fontSize: 13.5 }} />
              <span style={{ position: 'absolute', left: 10, color: C.muted, fontSize: 15, pointerEvents: 'none' }}>🔍</span>
              {materialesBusqueda && <button onClick={() => setMaterialesBusqueda('')} style={{ position: 'absolute', right: 10, color: C.muted }}><X size={14} /></button>}
            </div>
            <div className="rounded-xl overflow-hidden mt-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              <table>
                <thead><tr>
                  <th onClick={() => setMaterialesOrden((o) => ({ campo: 'nombre', dir: o.campo === 'nombre' ? -o.dir : 1 }))} style={{ cursor: 'pointer' }}>Material {materialesOrden.campo === 'nombre' ? (materialesOrden.dir === 1 ? '↑' : '↓') : ''}</th>
                  <th>Unidad</th>
                  <th onClick={() => setMaterialesOrden((o) => ({ campo: 'costoUnitario', dir: o.campo === 'costoUnitario' ? -o.dir : 1 }))} style={{ cursor: 'pointer' }}>Costo promedio {materialesOrden.campo === 'costoUnitario' ? (materialesOrden.dir === 1 ? '↑' : '↓') : ''}</th>
                  <th onClick={() => setMaterialesOrden((o) => ({ campo: 'stock', dir: o.campo === 'stock' ? -o.dir : 1 }))} style={{ cursor: 'pointer' }}>Stock {materialesOrden.campo === 'stock' ? (materialesOrden.dir === 1 ? '↑' : '↓') : ''}</th>
                  <th>Valor en inventario</th><th>Estado</th><th></th>
                </tr></thead>
                <tbody>
                  {materialesPag.paginaActual.map((m) => (
                    <tr key={m.id} style={{ opacity: m.activo === false ? 0.5 : 1 }}>
                      <td><input defaultValue={m.nombre} onBlur={(e) => { if (e.target.value.trim() && e.target.value.trim() !== m.nombre) updateMaterial(m.id, 'nombre', e.target.value.trim()); }} style={{ ...inputStyle, fontSize: 12.5, padding: '4px 6px', width: 160, border: 'none', backgroundColor: 'transparent' }} /></td>
                      <td><input defaultValue={m.unidad} onBlur={(e) => { if (e.target.value.trim() && e.target.value.trim() !== m.unidad) updateMaterial(m.id, 'unidad', e.target.value.trim()); }} style={{ ...inputStyle, fontSize: 12.5, padding: '4px 6px', width: 80, border: 'none', backgroundColor: 'transparent' }} /></td>
                      ...
                      <td>
                        {m.activo === false ? (
                          <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ backgroundColor: '#FDE8E8', color: C.coral }}>De baja</span>
                        ) : m.stock < 10
                      ...
                      <td>
                        <button onClick={() => toggleMaterialActivo(m.id)} title={m.activo === false ? 'Reactivar' : 'Dar de baja (deja de ofrecerse para nuevas órdenes)'}>
                          {m.activo === false ? <Check size={14} color={C.cyanDark} /> : <ShieldAlert size={14} color={C.amber} />}
                        </button>
                      </td>
                    </tr>
                  ))}
                  {materialesFiltrados.length === 0 && <tr><td colSpan={7} style={{ color: C.muted, textAlign: 'center', padding: 24 }}>Sin materiales que coincidan con la búsqueda.</td></tr>}
                </tbody>
              </table>
              <Paginador p={materialesPag} />
            </div>
          </div>
        )}
        ...
        {tab === 'kardex' && (
          <div>
            ...
            {kardexAction === 'entrada' && <EntradaForm materials={materials.filter((m) => m.activo !== false)} defaultMaterial={kardexMaterial} centros={centros} onSubmit={registerEntrada} />}
            {kardexAction === 'salida' && <SalidaForm materials={materials.filter((m) => m.activo !== false)} defaultMaterial={kardexMaterial} centros={centros} orders={orders.filter((o) => o.estado !== 'cancelada')} onSubmit={registerSalida} />}
        ...
        {tab === 'facturacion' && (() => {
          ...
          <div>
            ...
            <div className="rounded-xl overflow-hidden mt-5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              ...
              <table>
                <thead><tr><th>Folio</th><th>Cliente</th><th>Fecha</th><th>Antigüedad</th><th>Monto</th><th>Estado de pago</th><th></th></tr></thead>
                <tbody>
                  {[...conDias].sort((a, b) => (a.estadoPago === 'pendiente' ? 0 : 1) - (b.estadoPago === 'pendiente' ? 0 : 1) || b.dias - a.dias).map((o) => (
                    <tr key={o.id}>
                      ...
                      <td><button onClick={() => abrirTrazabilidad(o.id)} title="Ver trazabilidad"><GitBranch size={14} color={C.cyanDark} /></button></td>
                    </tr>
                  ))}
                  {conDias.length === 0 && <tr><td colSpan={7} style={{ color: C.muted, textAlign: 'center', padding: 24 }}>Aún no hay órdenes con ingreso reconocido.</td></tr>}
                </tbody>
                {conDias.length > 0 && (
            ...
            <div className="rounded-xl overflow-hidden mt-5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              ...
              <table>
                <thead><tr><th>Folio</th><th>Cliente</th><th>Concepto</th><th>Categoría de ingreso</th><th>Fecha</th><th>Monto</th><th>Estado</th><th></th></tr></thead>
                <tbody>
                  {facturasManualesPag.paginaActual.map((f) => (
                    <tr key={f.id}>
                      ...
                      <td><button onClick={() => abrirTrazabilidad(f.id)} title="Ver trazabilidad"><GitBranch size={14} color={C.cyanDark} /></button></td>
                    </tr>
                  ))}
                  {facturasManuales.length === 0 && <tr><td colSpan={8} style={{ color: C.muted, textAlign: 'center', padding: 20 }}>Sin facturas excepcionales — así debe ser la mayoría del tiempo.</td></tr>}
                </tbody>
              ...
              <Paginador p={facturasManualesPag} />
            </div>
          </div>
          );
        })()}
        ...
        {tab === 'cxp' && (
          <div>
            ...
            {showCxpForm && <CxpForm proveedores={proveedores.filter((p) => p.activo !== false)} gastoCategorias={gastoCategorias} materials={materials.filter((m) => m.activo !== false)} onSubmit={(data) => { addCxp(data); setShowCxpForm(false); }} />}

            <div className="flex items-center gap-1.5 mt-5" style={{ position: 'relative', maxWidth: 380 }}>
              <input value={cxpBusqueda} onChange={(e) => setCxpBusqueda(e.target.value)} placeholder="Buscar por concepto o proveedor..." style={{ ...inputStyle, width: '100%', paddingLeft: 32, fontSize: 13.5 }} />
              <span style={{ position: 'absolute', left: 10, color: C.muted, fontSize: 15, pointerEvents: 'none' }}>🔍</span>
              {cxpBusqueda && <button onClick={() => setCxpBusqueda('')} style={{ position: 'absolute', right: 10, color: C.muted }}><X size={14} /></button>}
            </div>
            <div className="rounded-xl overflow-hidden mt-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              <table>
                <thead><tr>
                  <th onClick={() => setCxpOrden((o) => ({ campo: 'proveedor', dir: o.campo === 'proveedor' ? -o.dir : 1 }))} style={{ cursor: 'pointer' }}>Proveedor {cxpOrden.campo === 'proveedor' ? (cxpOrden.dir === 1 ? '↑' : '↓') : ''}</th>
                  <th>Concepto</th><th>Tipo</th>
                  <th onClick={() => setCxpOrden((o) => ({ campo: 'fecha', dir: o.campo === 'fecha' ? -o.dir : 1 }))} style={{ cursor: 'pointer' }}>Fecha {cxpOrden.campo === 'fecha' ? (cxpOrden.dir === 1 ? '↑' : '↓') : ''}</th>
                  <th onClick={() => setCxpOrden((o) => ({ campo: 'vencimiento', dir: o.campo === 'vencimiento' ? -o.dir : 1 }))} style={{ cursor: 'pointer' }}>Vencimiento {cxpOrden.campo === 'vencimiento' ? (cxpOrden.dir === 1 ? '↑' : '↓') : ''}</th>
                  <th onClick={() => setCxpOrden((o) => ({ campo: 'monto', dir: o.campo === 'monto' ? -o.dir : 1 }))} style={{ cursor: 'pointer' }}>Monto {cxpOrden.campo === 'monto' ? (cxpOrden.dir === 1 ? '↑' : '↓') : ''}</th>
                  <th>Estado</th><th></th>
                </tr></thead>
                <tbody>
                  {cxpPagFiltrada.paginaActual.map((c) => (
                    <tr key={c.id}>
                      <td onClick={() => setProveedorSel(c.proveedorId)} style={{ cursor: 'pointer', color: C.cyanDark, textDecoration: 'underline' }}>{proveedorName(c.proveedorId)}</td>
                      <td>{c.concepto}</td>
                      ...
                      <td><button onClick={() => abrirTrazabilidad(c.id)} title="Ver trazabilidad"><GitBranch size={14} color={C.cyanDark} /></button></td>
                    </tr>
                  ))}
                  {cxpFiltradas.length === 0 && <tr><td colSpan={8} style={{ color: C.muted, textAlign: 'center', padding: 24 }}>Sin cuentas por pagar que coincidan con la búsqueda.</td></tr>}
                </tbody>
                {cxpFiltradas.length > 0 && (
                  <tfoot><tr><td colSpan={5} style={{ fontWeight: 700 }}>Total ({cxpFiltradas.length})</td><td style={{ fontFamily: 'JetBrains Mono, monospace', fontWeight: 700 }}>{fmt(cxpFiltradas.reduce((s, c) => s + c.monto, 0))}</td><td></td><td></td></tr></tfoot>
                )}
              </table>
              <Paginador p={cxpPagFiltrada} />
            </div>
          </div>
        )}
        ...
        {tab === 'proveedores' && (
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-5" style={{ position: 'relative', maxWidth: 380 }}>
              <input value={proveedoresBusqueda} onChange={(e) => setProveedoresBusqueda(e.target.value)} placeholder="Buscar por nombre o contacto..." style={{ ...inputStyle, width: '100%', paddingLeft: 32, fontSize: 13.5 }} />
              <span style={{ position: 'absolute', left: 10, color: C.muted, fontSize: 15, pointerEvents: 'none' }}>🔍</span>
              {proveedoresBusqueda && <button onClick={() => setProveedoresBusqueda('')} style={{ position: 'absolute', right: 10, color: C.muted }}><X size={14} /></button>}
            </div>
            <div className="grid grid-cols-2 gap-3 mt-4">
              {proveedoresPag.paginaActual.map((p) => {
              ...
              {proveedoresFiltrados.length === 0 && <div style={{ color: C.muted, fontSize: 13, gridColumn: '1 / -1', textAlign: 'center', padding: 24 }}>Sin proveedores que coincidan con la búsqueda.</div>}
            </div>
            <Paginador p={proveedoresPag} />
          </div>
        )}
        ...
        {tab === 'activos' && (
          <div>
            ...
            <div className="rounded-xl overflow-hidden mt-5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              <table>
                <thead><tr><th>Activo</th><th>Categoría</th><th>Centro de costo</th><th>Responsable</th><th>Fecha compra</th><th>Costo</th><th>Dep. mensual</th><th>Dep. contabilizada</th><th>Valor en libros contable</th><th></th></tr></thead>
                <tbody>
                  {activos.map((a) => {
                    const d = depreciacionInfo(a);
                    const contab = depContabilizada(a.supabaseId);
                    return (
                      <tr key={a.id} style={{ opacity: a.activo === false ? 0.5 : 1 }}>
                        <td><input defaultValue={a.nombre} onBlur={(e) => { if (e.target.value.trim() && e.target.value.trim() !== a.nombre) updateActivo(a.id, 'nombre', e.target.value.trim()); }} style={{ ...inputStyle, fontSize: 12.5, padding: '4px 6px', width: 130, border: 'none', backgroundColor: 'transparent' }} /></td>
                        <td style={{ fontSize: 12 }}>{a.categoria}{a.activo === false && <span className="text-xs font-bold px-1.5 py-0.5 rounded ml-1" style={{ backgroundColor: '#FDE8E8', color: C.coral }}>BAJA</span>}</td>
                        ...
                        <td>
                          <div className="flex gap-2">
                            <button onClick={() => toggleActivoBaja(a.id)} title={a.activo === false ? 'Reactivar' : 'Dar de baja'}>
                              {a.activo === false ? <Check size={14} color={C.cyanDark} /> : <ShieldAlert size={14} color={C.amber} />}
                            </button>
                            <button onClick={() => deleteActivo(a.id)} title="Eliminar"><Trash2 size={14} color={C.coral} /></button>
                          </div>
                        </td>
                      </tr>
                    );
                  })}
                  {activos.length === 0 && <tr><td colSpan={10} style={{ color: C.muted, textAlign: 'center', padding: 24 }}>Sin activos registrados.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
              ...
              <table>
                ...
                <tbody>
                  {[...depreciaciones].sort((a, b) => b.periodo.localeCompare(a.periodo)).map((d) => {
                    const a = activos.find((x) => x.supabaseId === d.activoId);
                    return (
                      <tr key={d.id}>
                        <td>{d.periodo}</td>
                        <td>{a?.nombre || '—'}</td>
              ...
              <table>
                ...
                <tbody>
                  {activos.filter((a) => a.categoria === 'Vehículo').map((a) => {
                    ...
                    const gastoMes = flotaLog.filter((f) => f.activoId === a.supabaseId && f.fecha?.startsWith(mesActual)).reduce((s, f) => s + f.monto, 0);
              ...
              <table>
                ...
                <tbody>
                  {[...flotaLog].reverse().map((f) => (
                    <tr key={f.id}>
                      <td>{f.fecha}</td>
                      <td>{activos.find((a) => a.supabaseId === f.activoId)?.nombre || '—'}</td>
                      <td>
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2 items-end flex-wrap">
                <button onClick={() => descargarCSVInforme('empleados', [['Nombre', 'Rol', 'Puesto', 'Teléfono', 'Salario mensual', 'Fecha de ingreso', 'Estado'], ...empleadosFiltrados.map((e) => [e.nombre, e.rol || 'Administrativo', e.puesto || '', e.telefono || '', e.salarioMensual || 0, e.fechaIngreso || '', e.activo === false ? 'Inactivo' : 'Activo'])])}
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-semibold" style={{ backgroundColor: '#fff', color: C.slate, border: `1px solid ${C.border}` }}>
                  <Download size={15} /> Exportar CSV
                </button>
            ...
            <div className="flex items-center gap-1.5 mt-5" style={{ position: 'relative', maxWidth: 380 }}>
              <input value={empleadosBusqueda} onChange={(e) => setEmpleadosBusqueda(e.target.value)} placeholder="Buscar por nombre o rol..." style={{ ...inputStyle, width: '100%', paddingLeft: 32, fontSize: 13.5 }} />
              <span style={{ position: 'absolute', left: 10, color: C.muted, fontSize: 15, pointerEvents: 'none' }}>🔍</span>
              {empleadosBusqueda && <button onClick={() => setEmpleadosBusqueda('')} style={{ position: 'absolute', right: 10, color: C.muted }}><X size={14} /></button>}
            </div>
            <div className="grid grid-cols-2 gap-3 mt-4">
              {empleadosPag.paginaActual.map((e) => (
              ...
              {empleadosFiltrados.length === 0 && <div style={{ gridColumn: '1 / -1', color: C.muted, textAlign: 'center', padding: 24 }}>Sin empleados que coincidan con la búsqueda.</div>}
            </div>
            <Paginador p={empleadosPag} />
            ...
            <div className="rounded-xl overflow-hidden mt-6" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              ...
              <table>
                <thead><tr><th>Empleado</th><th>Periodo</th><th>Fecha de pago</th><th>Bruto</th><th>Descuentos</th><th>Neto a pagar</th><th>Estado</th><th></th></tr></thead>
                <tbody>
                  {nominaPag.paginaActual.map((n) => (
                    <tr key={n.id}>
                      ...
                      <td><button onClick={() => abrirTrazabilidad(n.id)} title="Ver trazabilidad"><GitBranch size={14} color={C.cyanDark} /></button></td>
                    </tr>
                  ))}
                  {nomina.length === 0 && <tr><td colSpan={8} style={{ color: C.muted, textAlign: 'center', padding: 20 }}>Sin nómina registrada todavía.</td></tr>}
                </tbody>
                {nomina.length > 0 && (
                  <tfoot>
                    <tr>
            ...
            <div className="rounded-xl overflow-hidden mt-6" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              ...
              <table>
                ...
                <tbody>
                  {anticiposPag.paginaActual.map((a) => (
                    <tr key={a.id}>
            ...
            <div className="rounded-xl overflow-hidden mt-6" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              ...
              <table>
                ...
                <tbody>
                  {prestamosPag.paginaActual.map((p) => (
                    <tr key={p.id}>
                        ...
                        <React.Fragment key={a.id}>
                          ...
                          <tr style={{ opacity: opacidad, textDecoration: tachado }}>
                            ...
                            <td rowSpan={4} style={{ verticalAlign: 'top', paddingTop: 6, whiteSpace: 'nowrap' }}>
                              {!a.anulado && (
                                <div className="flex gap-2">
                                  {extraerFolioDocumento(a.glosa) && <button onClick={() => abrirTrazabilidad(extraerFolioDocumento(a.glosa))} title="Ver trazabilidad"><GitBranch size={14} color={C.cyanDark} /></button>}
        ...
        {tab === 'agenda' && (() => {
          ...
          const coincideBusqueda = (o) => {
            if (!agendaBusqueda.trim()) return true;
            const q = agendaBusqueda.trim().toLowerCase();
            return o.id.toLowerCase().includes(q) || clientName(o.clienteId).toLowerCase().includes(q) || (o.direccion || '').toLowerCase().includes(q);
          };
          return (
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-4" style={{ position: 'relative', maxWidth: 380 }}>
                <input value={agendaBusqueda} onChange={(e) => setAgendaBusqueda(e.target.value)} placeholder="Buscar por orden, cliente o ubicación..."
                  style={{ ...inputStyle, width: '100%', paddingLeft: 32, fontSize: 13.5 }} />
                <span style={{ position: 'absolute', left: 10, color: C.muted, fontSize: 15, pointerEvents: 'none' }}>🔍</span>
                {agendaBusqueda && (
                  <button onClick={() => setAgendaBusqueda('')} style={{ position: 'absolute', right: 10, color: C.muted }}><X size={14} /></button>
                )}
              </div>
              ...
              <div className="md:hidden mt-4">
                <div className="flex gap-2 mb-3">
                  <button onClick={() => cambiarVistaAgendaMovil('dia')} className="flex-1 py-2 rounded-lg text-sm font-semibold" style={{ backgroundColor: vistaAgendaMovil === 'dia' ? C.navy : '#fff', color: vistaAgendaMovil === 'dia' ? '#fff' : C.slate, border: `1px solid ${C.border}` }}>Día</button>
                ...
                {vistaAgendaMovil === 'dia' && (() => {
                  ...
                  const citas = propias.filter((o) => o.fecha === diaAgendaSel && o.estado !== 'cancelada' && o.estado !== 'por_validar').filter(coincideBusqueda).sort((a, b) => (a.hora || '99').localeCompare(b.hora || '99'));
                  const d = new Date(diaAgendaSel + 'T00:00:00');
                  const nombreDia = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'][d.getDay()];
                  const moverDia = (delta) => { const nd = new Date(d); nd.setDate(nd.getDate() + delta); setDiaAgendaSel(nd.toISOString().slice(0, 10)); };
                  const esHoy = diaAgendaSel === new Date().toISOString().slice(0, 10);
                  return (
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <button onClick={() => moverDia(-1)} style={{ color: C.cyanDark, fontSize: 15, padding: '4px 8px' }}>←</button>
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 14, color: C.slate }}>{nombreDia} {d.getDate()}/{d.getMonth() + 1}</div>
                          {!esHoy && <button onClick={() => setDiaAgendaSel(new Date().toISOString().slice(0, 10))} style={{ fontSize: 10.5, color: C.cyanDark }}>Ir a hoy</button>}
                        </div>
                        <button onClick={() => moverDia(1)} style={{ color: C.cyanDark, fontSize: 15, padding: '4px 8px' }}>→</button>
                      </div>
                      <div className="rounded-xl overflow-hidden" style={{ backgroundColor: C.card, border: `1px solid ${esHoy ? C.cyanDark : C.border}` }}>
                        <div style={{ backgroundColor: C.navy, color: '#fff', fontSize: 12, fontWeight: 600, padding: '7px 12px' }}>{citas.length} cita{citas.length === 1 ? '' : 's'}</div>
                        {citas.length === 0 ? (
                          <div style={{ padding: '20px 12px', color: C.muted, fontSize: 12.5, textAlign: 'center' }}>Sin citas para este día</div>
                        ) : citas.map((o, ci) => (
                          <div key={o.id} onClick={() => { setAgendaSel(o.id); setAgendaEdit(false); }} className="flex gap-2.5"
                            style={{ padding: '12px', borderTop: ci > 0 ? `1px solid ${C.border}` : 'none', cursor: 'pointer' }}>
                            <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 13, color: C.muted, width: 44, flexShrink: 0 }}>{o.hora || '—'}</div>
                            <div style={{ flex: 1 }}>
                              <div style={{ fontSize: 13, color: C.slate, fontWeight: 500 }}>{o.direccion || o.tipoServicio}</div>
                              <div style={{ fontSize: 11.5, color: C.muted, marginTop: 1 }}>{o.tipoServicio}</div>
                            </div>
                            <div style={{ backgroundColor: estadoColor[o.estado] + '22', color: estadoColor[o.estado], fontSize: 10.5, fontWeight: 600, padding: '3px 8px', borderRadius: 6, height: 'fit-content' }}>{ESTADOS[o.estado]?.label}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  return (
                    <div className="flex flex-col gap-3">
                      {dias.map((d, i) => {
                        ...
                        const citas = propias.filter((o) => o.fecha === fechaStr && o.estado !== 'cancelada' && o.estado !== 'por_validar').filter(coincideBusqueda).sort((a, b) => (a.hora || '99').localeCompare(b.hora || '99'));
                ...
                {vistaAgendaMovil === 'mes' && (() => {
                  ...
                  const citasDelDiaSel = propias.filter((o) => o.fecha === diaMesSel && o.estado !== 'cancelada' && o.estado !== 'por_validar').filter(coincideBusqueda).sort((a, b) => (a.hora || '99').localeCompare(b.hora || '99'));
        ...
        {tab === 'ventas' && (() => {
          ...
          <div>
            ...
            {showVentaForm && <VentaForm clients={clients.filter((c) => c.activo !== false)} materials={materials.filter((m) => m.activo !== false)} onSubmit={(data) => { addVenta(data); setShowVentaForm(false); }} />}
            ...
            <div className="flex items-center gap-1.5 mt-5" style={{ position: 'relative', maxWidth: 380 }}>
              <input value={ventasBusqueda} onChange={(e) => setVentasBusqueda(e.target.value)} placeholder="Buscar por folio o cliente..." style={{ ...inputStyle, width: '100%', paddingLeft: 32, fontSize: 13.5 }} />
              <span style={{ position: 'absolute', left: 10, color: C.muted, fontSize: 15, pointerEvents: 'none' }}>🔍</span>
              {ventasBusqueda && <button onClick={() => setVentasBusqueda('')} style={{ position: 'absolute', right: 10, color: C.muted }}><X size={14} /></button>}
            </div>
            <div className="rounded-xl overflow-hidden mt-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              <table>
                <thead><tr><th>Folio</th><th>Cliente</th><th>Fecha</th><th>Productos</th><th>Total</th><th>Margen</th><th>Cobro</th><th></th></tr></thead>
                <tbody>
                  {ventasPagFiltrada.paginaActual.map((v) => (
                    <tr key={v.id} style={{ opacity: v.anulada ? 0.5 : 1 }}>
                      ...
                      <td>
                        <div className="flex gap-2">
                          <button onClick={() => abrirTrazabilidad(v.id)} title="Ver trazabilidad"><GitBranch size={14} color={C.cyanDark} /></button>
                          {!v.anulada && v.estadoPago !== 'pagada' && <button onClick={() => anularVenta(v.id)} title="Anular venta"><X size={14} color={C.coral} /></button>}
                        </div>
                      </td>
                    </tr>
                  ))}
                  {ventasFiltradas.length === 0 && <tr><td colSpan={8} style={{ color: C.muted, textAlign: 'center', padding: 24 }}>{ventas.length === 0 ? 'Sin ventas registradas. Da de alta productos en Materiales y registra tu primera venta.' : 'Sin ventas que coincidan con la búsqueda.'}</td></tr>}
                </tbody>
...
function CxpForm({ proveedores, gastoCategorias, materials, onSubmit }) {
  ...
  const [avisoCxp, setAvisoCxp] = useState('');
  ...
  return (
    <div className="rounded-xl p-4 mt-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
      ...
      <button onClick={() => {
        if (!concepto) return;
        if (vencimiento < fecha) { setAvisoCxp('La fecha de vencimiento no puede ser antes de la fecha de la compra.'); return; }
        if (tipo === 'gasto' && (monto === '' || Number(monto) <= 0)) { setAvisoCxp('Escribe un monto mayor a cero.'); return; }
        setAvisoCxp('');
      ...
      {avisoCxp && <div style={{ fontSize: 12, color: C.coral, fontWeight: 600, marginTop: 6 }}>{avisoCxp}</div>}
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl p-5 mt-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
      <div className="grid grid-cols-3 gap-3">
        ...
        <Field label="Fecha"><input type="date" style={inputStyle} value={fecha} onChange={(e) => setFecha(e.target.value)} /></Field>
        <Field label="Precio al cliente ($)"><input type="number" min="0" style={inputStyle} value={precioCliente} onChange={(e) => setPrecioCliente(e.target.value)} /></Field>
        <Field label="Costo mano de obra ($)"><input type="number" min="0" style={inputStyle} value={costoManoObra} onChange={(e) => setCostoManoObra(e.target.value)} /></Field>
      </div>
      ...
      <button
        onClick={() => {
          if (!direccion.trim()) { setAvisoOrden('Escribe una dirección para la orden.'); return; }
          if (Number(costoManoObra) < 0 || Number(precioCliente) < 0) { setAvisoOrden('El precio y el costo de mano de obra no pueden ser negativos.'); return; }
          setAvisoOrden('');
          onSubmit({ clienteId, tecnicoId, proyectoId: proyectoId || null, centroCostoId, tipoServicio, direccion, fecha, costoManoObra: Number(costoManoObra) || 0, precioCliente: Number(precioCliente) || 0, materialesUsados: items, partidas });
        }}
        className="mt-4 px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ backgroundColor: C.cyanDark }}>
        Crear orden de trabajo
      </button>
      {avisoOrden && <div style={{ fontSize: 12, color: C.coral, fontWeight: 600, marginTop: 6 }}>{avisoOrden}</div>}
    </div>
  );
}
