    ...
    <div style={{ fontFamily: 'Inter, sans-serif', backgroundColor: C.bg, minHeight: '100vh', display: 'flex' }}>
      ...
      <div className="flex-1 p-4 md:p-8" style={{ maxWidth: 1220, minWidth: 0, overflowX: 'hidden' }}>
        ...
        {trazaOrden && (() => {
          ...
          const subRaiz = !raiz ? '—'
            : tipoRaiz === 'orden' ? `${raiz.fecha} · ${empleadoName(raiz.tecnicoId)}`
            : tipoRaiz === 'venta' ? `${raiz.fecha} · ${fmt(raiz.total)}`
            : tipoRaiz === 'cxp' ? `${raiz.fecha} · ${proveedorName(raiz.proveedorId)}`
            : tipoRaiz === 'nomina' ? `${raiz.fechaPago} · ${empleadoName(raiz.empleadoId)}` : '';
          const pasos = [
            { tipo: tipoRaiz, color: C.cyan, titulo: `${etiquetaRaiz} ${trazaOrden}`, sub: subRaiz, doc: raiz },
            ...aRel.map((a) => ({ tipo: 'asiento', color: '#8E6FCB', titulo: a.id, sub: `${a.fecha} · ${a.glosa}`, doc: a })),
            ...kRel.map((k) => ({ tipo: 'kardex', color: C.amber, titulo: k.id, sub: `${k.fecha} · ${k.tipo === 'entrada' ? '+' : '-'}${k.cantidad}`, doc: k })),
          ];
          return (
            <div style={{ position: 'fixed', inset: 0, zIndex: 65, display: 'flex', justifyContent: 'flex-end' }} onClick={() => setTrazaOrden(null)}>
              <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(11,22,34,0.35)' }} />
              <div onClick={(e) => e.stopPropagation()} style={{ position: 'relative', width: 320, maxWidth: '90vw', backgroundColor: C.navy, padding: '20px 18px', overflowY: 'auto' }}>
                <div className="flex items-center justify-between mb-1">
                  <div style={{ color: '#fff', fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 14 }}>Trazabilidad</div>
                  <button onClick={() => setTrazaOrden(null)}><X size={18} color="#93A5B8" /></button>
                </div>
                <div style={{ color: '#93A5B8', fontSize: 11.5, marginBottom: 16 }}>Orden {trazaOrden} — flujo completo</div>

                <div style={{ position: 'relative', paddingLeft: 20 }}>
                  <div style={{ position: 'absolute', left: 5, top: 6, bottom: 6, width: 2, backgroundColor: '#2A3B4D' }} />
                  {pasos.map((p, i) => (
                    <div key={i} onClick={() => p.doc && revisarDocumentoDeTraza(p.tipo, p.doc)} className="mb-4" style={{ position: 'relative', cursor: p.doc ? 'pointer' : 'default' }}>
                      <div style={{ position: 'absolute', left: -20, top: 2, width: 12, height: 12, borderRadius: '50%', backgroundColor: p.color }} />
                      <div style={{ color: '#fff', fontSize: 12, fontWeight: 600 }}>{p.titulo}</div>
                      <div style={{ color: '#93A5B8', fontSize: 10.5 }}>{p.sub}</div>
                      {p.doc && <div style={{ color: p.color, fontSize: 10, marginTop: 1 }}>Clic para revisar →</div>}
                    </div>
                  ))}
                  {pasos.length === 1 && <div style={{ color: '#7C93A8', fontSize: 11.5 }}>Aún no hay comprobantes ni movimientos ligados a esta orden.</div>}
                </div>
              </div>
            </div>
          );
