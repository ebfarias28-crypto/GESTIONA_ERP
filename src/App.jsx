...
const MODULOS_PERMISOS = [
  ...
  { grupo: 'Otros módulos', items: [['activos', 'Activos fijos'], ['facturacion', 'Facturación'], ['clientes', 'Clientes'], ['cxp', 'Cuentas por pagar'], ['inventario', 'Materiales'], ['kardex', 'Kardex'], ['rh', 'Empleados'], ['tecnicos', 'Técnicos'], ['bonos', 'Bonos por productividad'], ['reportes', 'Reportes']] },
];
...
const DEFAULT_ROLES_CONFIG = {
  ...
  Contabilidad: ['dashboard', 'kpi_financieros', 'kpi_ordenes', 'kpi_cartera', 'contab_plan', 'contab_comprobantes', 'contab_conciliacion', 'contab_informes', 'ingresos_gastos', 'contab_config', 'facturacion', 'clientes', 'cxp', 'reportes', 'activos'],
...
export default function App() {
  ...
  const setPermisoNivel = (rol, key, nivel) => {
    setRolesConfig((prev) => {
      const arr = (prev[rol] || []).filter((k) => k !== key);
      return { ...prev, [rol]: nivel === 'editar' ? [...arr, key] : arr };
    });
    setRolesLectura((prev) => {
      const arr = (prev[rol] || []).filter((k) => k !== key);
      return { ...prev, [rol]: nivel === 'ver' ? [...arr, key] : arr };
    });
  };
  const toggleOcultarValores = (rol, key) => {
    setRolesOcultarValores((prev) => {
      const arr = prev[rol] || [];
      return { ...prev, [rol]: arr.includes(key) ? arr.filter((k) => k !== key) : [...arr, key] };
    });
  };
  ...
  const [kardexLog, setKardexLog] = useState(SEED_KARDEX);
  ...
  const [kardexMaterial, setKardexMaterial] = useState(INIT_MATERIALS[0]?.id || '');
  ...
  const [proveedores, setProveedores] = useState(SEED_PROVEEDORES);
  const [cxp, setCxp] = useState(SEED_CXP);
  const [showProvForm, setShowProvForm] = useState(false);
  const [showCxpForm, setShowCxpForm] = useState(false);
  const [activos, setActivos] = useState(SEED_ACTIVOS);
  ...
  const [nomina, setNomina] = useState(SEED_NOMINA);
  ...
  const [asientos, setAsientos] = useState(SEED_ASIENTOS);
  const [showAsientoForm, setShowAsientoForm] = useState(false);
  const [editingAsiento, setEditingAsiento] = useState(null);
  const [cuentas, setCuentas] = useState(SEED_PLAN_CUENTAS);
  const [centros, setCentros] = useState(SEED_CENTROS_COSTO);
  const [gastoCategorias, setGastoCategorias] = useState(SEED_GASTO_CATEGORIAS);
  const [catalogoPartidas, setCatalogoPartidas] = useState(SEED_PARTIDAS_FTTH);
  const [tiposServicio, setTiposServicio] = useState(['Instalación residencial', 'Instalación empresarial', 'Empalme de fibra', 'Mantenimiento / reparación']);
  const [nuevoTipoServicio, setNuevoTipoServicio] = useState('');
  const [ingresoCategorias, setIngresoCategorias] = useState(SEED_INGRESO_CATEGORIAS);
  ...
  const [cuentaMayor, setCuentaMayor] = useState(SEED_PLAN_CUENTAS[0].codigo);
  const [mayorCentro, setMayorCentro] = useState('todos');
  const [contabVista, setContabVista] = useState('estado_resultados');
  const [expandedGroup, setExpandedGroup] = useState('contab_group');
  ...
  const revenue = useMemo(() => completedOrders.reduce((s, o) => s + Number(o.precioCliente || 0), 0), [completedOrders]);
  const costs = useMemo(() => activeOrders.reduce((s, o) => s + orderCost(o), 0), [activeOrders, materials]);
  ...
  const postAsiento = (fecha, glosa, cuentaDebe, monto, cuentaHaber, centroCostoId = null) => {
    ...
    setAsientos((prev) => [...prev, {
      id: 'AS' + String(prev.length + 1).padStart(3, '0'), fecha: fechaFinal, glosa: glosaFinal,
  ...
  const avisoFondos = (monto, concepto) => {
    ...
    if (Number(monto) > saldo) {
      notificar(usuariosAdmin(), 'aviso_fondos', 'Aviso de fondos', `El pago de ${concepto} (${fmt(Number(monto))}) dejó Bancos en ${fmt(saldo - Number(monto))}.`);
    }
  };
  ...
  const crearOrdenesImportadas = () => {
    const seleccionadas = importPreview.filter((p) => p.incluir);
    if (seleccionadas.length === 0) return;
    let lista = [...orders];
    seleccionadas.forEach((p) => {
      const folio = nextFolio(lista);
      const orden = {
        id: folio, proyectoId: null, centroCostoId: p.centroCostoId, clienteId: p.clienteId, tecnicoId: p.tecnicoId,
        tipoServicio: 'Activación FTTH', direccion: p.direccion, referencia: [p.referencia, p.wr && `WR ${p.wr}`].filter(Boolean).join(' · '),
        notas: p.notas, squadra: p.squadra || null, idRisorsa: p.idRisorsa || null,
        building: p.building || null, cnoBuilding: p.cnoBuilding || null, roeBuilding: p.roeBuilding || null,
        fecha: p.fecha, hora: p.hora || null, estado: 'pendiente', materialesUsados: [],
        costoManoObra: Number(p.costoManoObra) || 0, precioCliente: Number(p.precioCliente) || 0,
        estadoPago: 'pendiente', estadoPagoTecnico: 'pendiente',
      };
      lista = [...lista, orden];
      if (orden.costoManoObra > 0) postAsiento(orden.fecha, `Mano de obra por pagar — orden ${folio}`, '6140', orden.costoManoObra, '2510', orden.centroCostoId);
    });
    setOrders(lista);
    setImportPreview([]);
    alert(`Se crearon ${seleccionadas.length} orden(es) de trabajo.`);
  };

  // ---------- Notificaciones in-app (simuladas en esta sesión; con Supabase se disparan aunque el destinatario no tenga la app abierta) ----------
  const usuariosPorEmpleado = (empleadoId) => usuarios.filter((u) => u.empleadoId === empleadoId);
  const usuariosAdmin = () => usuarios.filter((u) => ['Administrador', 'Contabilidad'].includes(u.rol) && (u.empresas === 'todas' || (u.empresas || []).includes(empresaActiva)));
  const notificar = (destinatarios, tipo, titulo, mensaje, ordenId) => {
    if (!destinatarios || destinatarios.length === 0) return;
    const ahora = new Date().toISOString();
    setNotificaciones((prev) => {
      const base = prev.map((x) => parseInt(String(x.id).replace('NT', ''), 10) || 0);
      const inicio = (base.length ? Math.max(...base) : 0) + 1;
      const nuevas = destinatarios.map((u, i) => ({ id: 'NT' + String(inicio + i).padStart(3, '0'), usuarioId: u.id, tipo, titulo, mensaje, ordenId: ordenId || null, fecha: ahora, leida: false }));
      const combinadas = [...nuevas, ...prev];
      // Límite automático: conserva como máximo las 50 más recientes por usuario (evita que la campana crezca sin fin)
      const destinatarioIds = new Set(destinatarios.map((u) => u.id));
      const otras = combinadas.filter((n) => !destinatarioIds.has(n.usuarioId));
      const podadas = [...destinatarioIds].flatMap((uid) => combinadas.filter((n) => n.usuarioId === uid).slice(0, 50));
      return [...podadas, ...otras].sort((a, b) => new Date(b.fecha) - new Date(a.fecha));
    });
  };

  const addOrderWithKardex = (data) => {
    ...
    const newEntries = [];
    (data.materialesUsados || []).forEach((mu) => {
      const idx = matSnapshot.findIndex((m) => m.id === mu.materialId);
      if (idx === -1) return;
      matSnapshot[idx].stock -= mu.cantidad;
      newEntries.push({
        id: 'K' + String(kardexLog.length + newEntries.length + 1).padStart(4, '0'),
        ...
        saldoCantidad: matSnapshot[idx].stock, saldoValor: matSnapshot[idx].stock * matSnapshot[idx].costoUnitario,
        centroCostoId: data.centroCostoId || null,
      });
    });
    setMaterials(matSnapshot);
    setKardexLog((prev) => [...prev, ...newEntries]);
    // Control de validación: si quien captura no es Administrador/Contabilidad, la orden entra "por validar"
    ...
    setOrders((prev) => [...prev, {
      id: folio, estadoPago: 'pendiente', estadoPagoTecnico: 'pendiente', ...data,
      estado: estadoInicial, validada: !requiereValidacion,
      capturadaPor: sesion?.nombre || 'Sistema', ingresoPosteado: false,
    }]);
  ...
  const validarOrden = (id, aprobar) => {
    ...
    if (aprobar && Number(ord.precioCliente) > 0 && !ord.ingresoPosteado) {
  ...
  const changeOrderEstado = (id, nuevoEstado) => {
    if (bloqueadoPorLectura()) return;
    ...
    if (nuevoEstado === 'cancelada' && ordActual && ordActual.estado !== 'cancelada') {
      ...
      if (salidas.length > 0 && !yaDevueltas) {
        ...
        salidas.forEach((s) => {
          ...
          entradas.push({
            id: 'K' + String(kardexLog.length + entradas.length + 1).padStart(4, '0'),
            ...
            saldoCantidad: matActualizados[idx].stock, saldoValor: matActualizados[idx].stock * matActualizados[idx].costoUnitario,
            centroCostoId: ordActual.centroCostoId || null,
          });
        });
    ...
    // Notifica a administración cuando el técnico marca la orden como completada
    if (nuevoEstado === 'completada' && ordActual && ordActual.estado !== 'completada') {
      notificar(usuariosAdmin(), 'orden_completada', 'Orden completada', `${sesion?.nombre || 'Un técnico'} completó la orden ${id}${ordActual.tecnicoId ? ` (${empleadoName(ordActual.tecnicoId)})` : ''}.`, id);
    }
  };
  ...
  const togglePagoCliente = (id) => {
    ...
    setOrders((prev) => prev.map((o) => {
      if (o.id !== id) return o;
      const nuevo = o.estadoPago === 'pagada' ? 'pendiente' : 'pagada';
  ...
  const togglePagoTecnico = (id) => {
    ...
    setOrders((prev) => prev.map((o) => {
      if (o.id !== id) return o;
      const nuevo = o.estadoPagoTecnico === 'pagado' ? 'pendiente' : 'pagado';
      const hoy = new Date().toISOString().slice(0, 10);
  ...
  const registerEntrada = ({ materialId, cantidad, costoUnitario, fecha, referencia, centroCostoId }) => {
    ...
    setKardexLog((prev) => [...prev, {
      id: 'K' + String(prev.length + 1).padStart(4, '0'), materialId, fecha, tipo: 'entrada',
      cantidad: qty, costoUnitario: cu, referencia: referencia || 'Compra',
      saldoCantidad: newStock, saldoValor: newStock * newCosto, centroCostoId: centroCostoId || null,
    }]);
  ...
  const registerSalida = ({ materialId, cantidad, fecha, referencia, centroCostoId }) => {
    const idx = materials.findIndex((m) => m.id === materialId);
    if (idx === -1) return;
    const m = materials[idx];
    ...
    if (qty <= 0) return;
    const updated = [...materials];
    updated[idx] = { ...m, stock: m.stock - qty };
    setMaterials(updated);
    setKardexLog((prev) => [...prev, {
      id: 'K' + String(prev.length + 1).padStart(4, '0'), materialId, fecha, tipo: 'salida',
      cantidad: qty, costoUnitario: m.costoUnitario, referencia: referencia || 'Consumo manual',
      saldoCantidad: m.stock - qty, saldoValor: (m.stock - qty) * m.costoUnitario, centroCostoId: centroCostoId || null,
    }]);
    postAsiento(fecha, `Consumo de material — ${m.nombre}${referencia ? ' (' + referencia + ')' : ''}`, '6135', qty * m.costoUnitario, '1435', centroCostoId || null);
  };

  const addMaterialWithKardex = (data) => {
    const id = nextId('M', materials);
    const stock = Number(data.stock) || 0;
    const costo = Number(data.costoUnitario) || 0;
    setMaterials((p) => [...p, { id, nombre: data.nombre, unidad: data.unidad, costoUnitario: costo, stock }]);
    setKardexLog((prev) => [...prev, {
      id: 'K' + String(prev.length + 1).padStart(4, '0'), materialId: id, fecha: new Date().toISOString().slice(0, 10),
      tipo: 'entrada', cantidad: stock, costoUnitario: costo, referencia: 'Alta de material — stock inicial',
      saldoCantidad: stock, saldoValor: stock * costo,
    }]);
  ...
  const addFacturaManual = (data) => {
    const monto = Number(data.monto) || 0;
    const cuentaIngreso = ingresoCategorias.find((c) => c.nombre === data.categoria)?.cuenta || '4200';
    setFacturasManuales((p) => [...p, { id: nextId('FE', p), ...data, monto, cuentaIngreso, estadoPago: 'pendiente' }]);
    postAsiento(data.fecha, `Factura excepcional — ${data.concepto}`, '1305', monto, cuentaIngreso, 'CC-ADM');
  };
  const toggleFacturaManualPago = (id) => setFacturasManuales((prev) => prev.map((f) => {
    if (f.id !== id) return f;
    const nuevo = f.estadoPago === 'pagada' ? 'pendiente' : 'pagada';
    const hoy = new Date().toISOString().slice(0, 10);
    if (nuevo === 'pagada') postAsiento(hoy, `Cobro factura excepcional — ${f.concepto}`, '1110', f.monto, '1305', 'CC-ADM');
    else postAsiento(hoy, `Reversión cobro factura excepcional — ${f.concepto}`, '1305', f.monto, '1110', 'CC-ADM');
    return { ...f, estadoPago: nuevo };
  }));
  ...
  const addVenta = (data) => {
    ...
    if (faltantes.length > 0) { alert('Stock insuficiente:\n' + faltantes.join('\n')); return; }
    const id = nextId('V', ventas);
    const totalVenta = data.lineas.reduce((s, l) => s + l.cantidad * l.precioUnitario, 0);
    // Descarga de inventario al costo promedio + costo de ventas
    const matActualizados = materials.map((m) => ({ ...m }));
    const salidas = [];
    let costoTotal = 0;
    data.lineas.forEach((l) => {
      ...
      const costoLinea = l.cantidad * matActualizados[idx].costoUnitario;
      costoTotal += costoLinea;
      salidas.push({
        id: 'K' + String(kardexLog.length + salidas.length + 1).padStart(4, '0'),
        materialId: l.materialId, fecha: data.fecha, tipo: 'salida', cantidad: l.cantidad,
        costoUnitario: matActualizados[idx].costoUnitario, referencia: `Venta ${id}`,
        saldoCantidad: matActualizados[idx].stock, saldoValor: matActualizados[idx].stock * matActualizados[idx].costoUnitario,
        centroCostoId: 'CC-OPE',
      });
    });
    setMaterials(matActualizados);
    setKardexLog((prev) => [...prev, ...salidas]);
    setVentas((p) => [...p, { id, clienteId: data.clienteId, fecha: data.fecha, lineas: data.lineas, total: totalVenta, costo: costoTotal, estadoPago: 'pendiente', anulada: false, usuario: sesion?.nombre }]);
    postAsiento(data.fecha, `Venta ${id}`, '1305', totalVenta, '4135', 'CC-OPE');
    if (costoTotal > 0) postAsiento(data.fecha, `Costo de ventas — ${id}`, '6135', costoTotal, '1435', 'CC-OPE');
  };
  const toggleVentaCobro = (id) => {
    if (bloqueadoPorLectura()) return;
    setVentas((prev) => prev.map((v) => {
      if (v.id !== id || v.anulada) return v;
      const nuevo = v.estadoPago === 'pagada' ? 'pendiente' : 'pagada';
      const hoy = new Date().toISOString().slice(0, 10);
      if (nuevo === 'pagada') postAsiento(hoy, `Cobro venta ${v.id}`, '1110', v.total, '1305', 'CC-OPE');
      else postAsiento(hoy, `Reversión cobro venta ${v.id}`, '1305', v.total, '1110', 'CC-OPE');
      return { ...v, estadoPago: nuevo };
    }));
  };
  const anularVenta = (id) => {
    ...
    pedirConfirmacion(`¿Anular la venta ${id}? Los productos regresarán al inventario y se revertirá la contabilidad.`, () => {
      // Devolver productos al inventario al costo de la salida
      const salidasVenta = kardexLog.filter((k) => k.tipo === 'salida' && k.referencia === `Venta ${id}`);
      const matActualizados = materials.map((m) => ({ ...m }));
      const entradas = [];
      salidasVenta.forEach((s) => {
        const idx = matActualizados.findIndex((m) => m.id === s.materialId);
        if (idx === -1) return;
        matActualizados[idx].stock += s.cantidad;
        entradas.push({
  ...
  const addEquipoComodato = (data) => {
    if (bloqueadoPorLectura()) return;
    if (equiposComodato.some((e) => e.serial === data.serial)) { alert('Ya existe un equipo con ese número de serie.'); return; }
    setEquiposComodato((p) => [...p, { id: nextId('EQ', p), ...data, estado: 'en_almacen', clienteId: null, fechaAsignacion: null }]);
  };
  const asignarEquipoComodato = (id, clienteId, ordenId) => {
    if (bloqueadoPorLectura()) return;
    setEquiposComodato((prev) => prev.map((e) => e.id === id ? { ...e, estado: 'asignado', clienteId, ordenId: ordenId || null, fechaAsignacion: new Date().toISOString().slice(0, 10) } : e));
  };
  const devolverEquipoComodato = (id) => {
    if (bloqueadoPorLectura()) return;
    setEquiposComodato((prev) => prev.map((e) => e.id === id ? { ...e, estado: 'en_almacen', clienteId: null, ordenId: null, fechaDevolucion: new Date().toISOString().slice(0, 10) } : e));
  };
  ...
  const addSquadra = (data) => {
    if (bloqueadoPorLectura()) return;
    setSquadras((p) => [...p, { id: nextId('SQ', p), nombre: data.nombre, miembros: data.miembros, activa: true }]);
  };
  ...
  const addFlotaRegistro = (tipo, data) => {
    if (bloqueadoPorLectura()) return;
    const activo = activos.find((a) => a.id === data.activoId);
    const monto = Number(data.monto) || 0;
    setFlotaLog((p) => [...p, { id: nextId('FL', p), tipo, ...data, monto }]);
    if (monto > 0) {
  ...
  const addMovimiento = (data) => {
    ...
    setMovimientos((p) => [...p, { id: nextId('MV', p), ...data, monto: Number(data.monto) || 0 }]);
    const monto = Number(data.monto) || 0;
  ...
  const addProveedor = (data) => setProveedores((p) => [...p, { id: nextId('PR', p), ...data }]);
  const addCxp = (data) => {
    if (bloqueadoPorLectura()) return;
    const monto = Number(data.monto) || 0;
    const id = nextId('CP', cxp);
    if (data.tipo === 'inventario' && data.materialId && Number(data.cantidad) > 0) {
      // Compra a crédito de inventario: alimenta el kardex (entrada) y actualiza el costo promedio del material
      const cantidad = Number(data.cantidad);
      const costoUnitario = monto / cantidad;
      const matActualizados = materials.map((m) => ({ ...m }));
      const idx = matActualizados.findIndex((m) => m.id === data.materialId);
      if (idx !== -1) {
        ...
        setMaterials(matActualizados);
        setKardexLog((prev) => [...prev, {
          id: 'K' + String(prev.length + 1).padStart(4, '0'), materialId: data.materialId, fecha: data.fecha, tipo: 'entrada',
          cantidad, costoUnitario, referencia: `Compra a crédito ${id}`,
          saldoCantidad: nuevoStock, saldoValor: nuevoStock * nuevoCostoProm, centroCostoId: 'CC-ADM',
        }]);
      }
    ...
    setCxp((p) => [...p, { id, estado: 'pendiente', ...data, monto }]);
  };
  const toggleCxpPago = (id) => {
    ...
    setCxp((prev) => prev.map((c) => {
      if (c.id !== id) return c;
      const nuevo = c.estado === 'pagada' ? 'pendiente' : 'pagada';
      const hoy = new Date().toISOString().slice(0, 10);
  ...
  // ---------- Activos fijos ----------
  const addActivo = (data) => {
    setActivos((p) => [...p, { id: nextId('AF', p), ...data, costo: Number(data.costo) || 0, valorResidual: Number(data.valorResidual) || 0, vidaUtilAnios: Number(data.vidaUtilAnios) || 1 }]);
    postAsiento(data.fechaCompra, `Compra de activo fijo — ${data.nombre}`, '1540', Number(data.costo) || 0, '1110', data.centroCostoId || null);
  };
  const depreciacionInfo = (a) => {
    const anios = Math.max(0, (new Date() - new Date(a.fechaCompra)) / (1000 * 60 * 60 * 24 * 365));
  ...
  const valorActivosLibros = activos.reduce((s, a) => s + depreciacionInfo(a).valorEnLibros, 0);
  ...
  const registrarDepreciacionMes = (periodo) => {
    activos.forEach((a) => {
      ...
      const cuentaDep = a.centroCostoId === 'CC-ADM' ? '5310' : '6310';
      postAsiento(`${periodo}-01`, `Depreciación ${periodo} — ${a.nombre}`, cuentaDep, monto, '1541', a.centroCostoId || null);
      setDepreciaciones((prev) => [...prev, { id: nextId('DEP', prev), activoId: a.id, periodo, fecha: `${periodo}-01`, monto }]);
    });
  };
  ...
  const toggleEmpleadoActivo = (id) => {
    if (bloqueadoPorLectura()) return;
    setEmpleados((prev) => prev.map((e) => e.id === id ? { ...e, activo: e.activo === false ? true : false } : e));
  };
  const updateEmpleado = (id, campo, valor) => setEmpleados((prev) => prev.map((e) => e.id === id ? { ...e, [campo]: campo.startsWith('salario') || campo.startsWith('tarifa') ? Number(valor) || 0 : valor } : e));
  const toggleClienteActivo = (id) => setClients((prev) => prev.map((c) => c.id === id ? { ...c, activo: c.activo === false ? true : false } : c));
  const updateCliente = (id, campo, valor) => setClients((prev) => prev.map((c) => c.id === id ? { ...c, [campo]: valor } : c));
  const toggleProveedorActivo = (id) => setProveedores((prev) => prev.map((p) => p.id === id ? { ...p, activo: p.activo === false ? true : false } : p));
  const updateProveedor = (id, campo, valor) => setProveedores((prev) => prev.map((p) => p.id === id ? { ...p, [campo]: valor } : p));
  const addEmpleado = (data) => setEmpleados((p) => [...p, { id: nextId(data.rol === 'Técnico' ? 'T' : 'E', p), ...data, rol: data.rol || 'Administrativo', salarioMensual: Number(data.salarioMensual) || 0, reportaA: data.reportaA || null }]);
  const empleadoName = (id) => empleados.find((e) => e.id === id)?.nombre || '—';
  // ---------- Anticipos de sueldo ----------
  const addAnticipo = (data) => {
    if (bloqueadoPorLectura()) return;
    const monto = Number(data.monto) || 0;
    avisoFondos(monto, `anticipo a ${empleadoName(data.empleadoId)}`);
    setAnticipos((p) => [...p, { id: nextId('AN', p), ...data, monto, estado: 'pendiente' }]);
    postAsiento(data.fecha, `Anticipo de sueldo — ${empleadoName(data.empleadoId)}`, '1310', monto, '1110', 'CC-ADM');
  };
  // ---------- Préstamos a empleados ----------
  const addPrestamo = (data) => {
    if (bloqueadoPorLectura()) return;
    ...
    setPrestamos((p) => [...p, { id: nextId('PR', p), empleadoId: data.empleadoId, fecha: data.fecha, motivo: data.motivo, montoTotal, cuotas, montoCuota: montoTotal / cuotas, saldoPendiente: montoTotal, cuotasPagadas: 0, estado: 'activo' }]);
    postAsiento(data.fecha, `Préstamo a empleado — ${empleadoName(data.empleadoId)}`, '1315', montoTotal, '1110', 'CC-ADM');
  };

  const addNomina = (data) => {
    if (bloqueadoPorLectura()) return;
    ...
    setNomina((p) => [...p, { id: nextId('NM', p), estado: 'pendiente', ...data, montoBruto: bruto, descuentoAnticipo, descuentoPrestamo, monto: neto }]);
    ...
    if (descuentoPrestamo > 0 && prestamoActivo) {
      ...
      setPrestamos((prev) => prev.map((p) => p.id === prestamoActivo.id ? { ...p, saldoPendiente: nuevoSaldo, cuotasPagadas: p.cuotasPagadas + 1, estado: nuevoSaldo <= 0.01 ? 'liquidado' : 'activo' } : p));
    }
  };
  ...
  const toggleNominaPago = (id) => {
    ...
    if (n.id !== id) return n;
    const nuevo = n.estado === 'pagado' ? 'pendiente' : 'pagado';
    const hoy = new Date().toISOString().slice(0, 10);
    if (nuevo === 'pagado') {
  ...
  const addAsiento = (data) => {
    ...
    setAsientos((p) => [...p, { id: nextId('AS', p), ...data, montoDebe: Number(data.montoDebe) || 0, montoHaber: Number(data.montoHaber) || 0 }]);
  };
  const updateAsiento = (id, data) => {
    ...
    setAsientos((prev) => prev.map((a) => a.id === id ? { ...a, ...data, montoDebe: Number(data.montoDebe) || 0, montoHaber: Number(data.montoHaber) || 0 } : a));
  };
  const anularAsiento = (id) => {
    ...
    pedirConfirmacion('¿Anular este comprobante? Dejará de afectar todos los reportes contables, pero quedará visible en el libro diario como anulado.', () => {
      setAsientos((prev) => prev.map((a) => a.id === id ? { ...a, anulado: true } : a));
    });
  };
  ...
  const updateCuentaNombre = (codigo, nombre) => setCuentas((prev) => prev.map((c) => c.codigo === codigo ? { ...c, nombre } : c));
  const updateCuentaSubtipo = (codigo, subtipo) => setCuentas((prev) => prev.map((c) => c.codigo === codigo ? { ...c, subtipo: subtipo || null } : c));
  const addCuenta = (data) => {
    if (cuentas.some((c) => c.codigo === data.codigo)) return;
    setCuentas((p) => [...p, { codigo: data.codigo, nombre: data.nombre, tipo: data.tipo, subtipo: data.subtipo || null, grupo: data.grupo || null }].sort((a, b) => a.codigo.localeCompare(b.codigo)));
  };
  const updateCuentaGrupo = (codigo, grupo) => setCuentas((prev) => prev.map((c) => c.codigo === codigo ? { ...c, grupo: grupo || null } : c));
  ...
  const addCentro = (data) => {
    if (centros.some((c) => c.id === data.id)) { alert('Ya existe un centro con ese código.'); return; }
    setCentros((p) => [...p, { id: data.id, nombre: data.nombre }]);
  };
  ...
  const erProrrateoInfo = (() => {
    ...
    const admAsientos = asientosVigentes.filter((a) => inRange(a.fecha) && a.centroCostoId === 'CC-ADM');
    ...
    const centrosOp = centros.filter((c) => c.id !== 'CC-ADM');
    let factor;
    if (prorrateoMetodo === 'equitativo') {
      factor = centrosOp.length > 0 ? 1 / centrosOp.length : 0;
    } else {
      const ingTotales = asientosVigentes.filter((a) => inRange(a.fecha) && a.centroCostoId !== 'CC-ADM' && cuentas.find((c) => c.codigo === a.cuentaHaber)?.grupo === 'ingresos_operativos').reduce((s, a) => s + a.montoHaber, 0)
        - asientosVigentes.filter((a) => inRange(a.fecha) && a.centroCostoId !== 'CC-ADM' && cuentas.find((c) => c.codigo === a.cuentaDebe)?.grupo === 'ingresos_operativos').reduce((s, a) => s + a.montoDebe, 0);
  ...
  const erTotalsFor = (from, to, centro) => {
    ...
    if (centro !== 'todos' && centro !== 'CC-ADM') {
      const filtradosAdm = asientosVigentes.filter((a) => enRango(a.fecha) && a.centroCostoId === 'CC-ADM');
      const saldoAdm = (codigo, tipo) => {
      ...
      const gastosAdmTotal = cuentas.filter((c) => c.grupo === 'gastos_generales').reduce((s, c) => s + saldoAdm(c.codigo, c.tipo), 0);
      const centrosOperativos = centros.filter((c) => c.id !== 'CC-ADM');
      if (prorrateoMetodo === 'equitativo') {
        ...
        const ingresosTotales = asientosVigentes.filter((a) => enRango(a.fecha) && a.centroCostoId !== 'CC-ADM')
          .filter((a) => cuentas.find((c) => c.codigo === a.cuentaHaber)?.grupo === 'ingresos_operativos').reduce((s, a) => s + a.montoHaber, 0)
          - asientosVigentes.filter((a) => enRango(a.fecha) && a.centroCostoId !== 'CC-ADM')
  ...
  const nav = [
    ...
    { key: 'activos', label: t('Activos fijos'), icon: Truck },
    {
    ...
    { key: 'cxp', label: t('Cuentas por pagar'), icon: Building2 },
    {
  ...
  const intentarLogin = async () => {
    setLoginError('');
    const { data, error } = await supabase.auth.signInWithPassword({ email: loginEmail.trim(), password: loginClave });
    if (error) { setLoginError('Correo o clave incorrectos.'); return; }
    const ahora = new Date().toISOString();
    ...
    if (s) setTab(s.preferencias?.pantallaInicio || (s.rol === 'Tecnico' ? 'agenda' : s.rol === 'Operaciones' ? 'ordenes' : 'dashboard'));
  };
    ...
    return (
      <div style={{ fontFamily: 'Inter, sans-serif', backgroundColor: C.navy, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
        ...
        <div className="rounded-2xl p-8" style={{ backgroundColor: '#fff', width: 380, maxWidth: '100%', boxShadow: '0 24px 60px rgba(0,0,0,0.35)' }}>
          ...
          <Field label="Correo"><input type="email" style={{ ...inputStyle, width: '100%' }} value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (authMode === 'login' ? intentarLogin() : intentarRegistro())} autoFocus={authMode === 'login'} /></Field>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 767px) {
          /* Las grillas de tarjetas (KPIs, resúmenes) pasan a una sola columna en celular */
      ...
      <div className="flex-1 p-4 md:p-8" style={{ maxWidth: 1220, minWidth: 0, overflowX: 'hidden' }}>
        ...
        <div className="flex justify-end items-center gap-2.5 mb-4 no-print" style={{ position: 'relative' }}>
                  ...
                  <div className="rounded-xl overflow-hidden" style={{ position: 'absolute', top: 50, right: 0, backgroundColor: '#fff', border: `1px solid ${C.border}`, boxShadow: '0 12px 32px rgba(0,0,0,0.14)', width: 340, zIndex: 40, maxHeight: 420, overflowY: 'auto' }}>
                    <div className="px-4 py-3 flex items-center justify-between" style={{ backgroundColor: C.navy }}>
                      ...
                      <div className="flex items-center gap-3">
                        {noLeidas > 0 && (
                          <button onClick={() => setNotificaciones((prev) => prev.map((n) => n.usuarioId === sesion.id ? { ...n, leida: true } : n))} style={{ color: C.cyan, fontSize: 11, fontWeight: 600 }}>Marcar todas leídas</button>
                        )}
                        {misNotifs.some((n) => n.leida) && (
                          <button onClick={() => setNotificaciones((prev) => prev.filter((n) => !(n.usuarioId === sesion.id && n.leida)))} style={{ color: '#9DB2C4', fontSize: 11, fontWeight: 600 }}>Limpiar leídas</button>
                        )}
                      </div>
                    </div>
          ...
          {userMenuOpen && (
            <div className="rounded-xl overflow-hidden" style={{ position: 'absolute', top: 52, right: 0, backgroundColor: '#fff', border: `1px solid ${C.border}`, boxShadow: '0 12px 32px rgba(0,0,0,0.14)', width: 220, zIndex: 40 }}>
              ...
              <button onClick={() => { setCuentaNombreEdit(sesion.nombre); setCuentaClaveNueva(''); setCuentaClaveConfirm(''); setCuentaMsg(''); setTab('cuenta'); setUserMenuOpen(false); }} className="w-full text-left px-4 py-2.5 text-sm" style={{ color: C.slate, borderTop: `1px solid ${C.border}` }}>{t('Cuenta')}</button>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-5 mt-5">
              <BarChart title="Utilidad por centro de costo" valueFmt={fmt}
                colorFn={(d) => d.value >= 0 ? C.cyanDark : C.coral}
                data={centros.filter((c) => c.id !== 'CC-ADM').map((c) => ({ label: c.nombre, value: centroCostoMetrics(c.id, ordersRango.filter((o) => o.estado === 'completada')).utilidad }))} />
            ...
            <div className="rounded-xl overflow-hidden mt-5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              ...
              <table>
                ...
                <tbody>
                  {centros.filter((c) => c.id !== 'CC-ADM').map((c) => {
          ...
          <div>
            ...
            {showProjectForm && (
              <div className="rounded-xl p-4 mt-4 flex flex-wrap gap-3 items-end" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
                <ProjectFormFields clients={clients} onSubmit={(data) => { setProjects((p) => [...p, { id: nextId('P', p), estado: 'activo', ...data }]); setShowProjectForm(false); }} />
              </div>
            )}
                ...
                return (
                  <div key={p.id} className="rounded-xl p-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
                    <div className="flex items-center justify-between">
                      ...
                      <select value={p.estado} onChange={(e) => setProjects((prev) => prev.map((x) => x.id === p.id ? { ...x, estado: e.target.value } : x))}
          ...
          <div>
            ...
            {showClientForm && (
              <SimpleForm fields={[{ key: 'nombre', label: 'Nombre / Razón social' }, { key: 'telefono', label: 'Teléfono' }, { key: 'direccion', label: 'Dirección' }]}
                onSubmit={(data) => { setClients((p) => [...p, { id: nextId('C', p), ...data }]); setShowClientForm(false); }} />
            )}
            <div className="grid grid-cols-2 gap-3 mt-5">
              {clients.map((c) => {
        ...
        {tab === 'kardex' && (
          <div>
            ...
            {kardexAction === 'salida' && <SalidaForm materials={materials} defaultMaterial={kardexMaterial} centros={centros} onSubmit={registerSalida} />}
        ...
        {/* FLOTA — dentro de Activos fijos: combustible, mantenimiento, seguro/revisión, multas */}
        {tab === 'activos' && activos.some((a) => a.categoria === 'Vehículo') && (
          <div className="mt-8 pt-6" style={{ borderTop: `1px solid ${C.border}` }}>
            <h2 style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: 17, fontWeight: 700, color: C.slate }}>Flota — control operativo de vehículos</h2>
            <p style={{ color: C.muted, fontSize: 12.5, marginTop: 2 }}>Vencimientos de seguro y revisión, y bitácora de combustible, mantenimiento y multas por unidad.</p>
          ...
          <div>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p style={{ color: C.muted, fontSize: 13, marginTop: 2 }}>Libro diario: cada comprobante es un asiento contable (debe = haber).</p>
              </div>
              <button onClick={() => setShowAsientoForm(!showAsientoForm)} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-white text-sm font-semibold" style={{ backgroundColor: C.navy }}>
                {showAsientoForm ? <X size={15} /> : <Plus size={15} />} {showAsientoForm ? 'Cerrar' : 'Nuevo comprobante'}
              </button>
            </div>

            {showAsientoForm && !editingAsiento && <AsientoForm cuentas={cuentas} centros={centros} onSubmit={(data) => { addAsiento(data); setShowAsientoForm(false); }} />}
            {editingAsiento && (
              <div>
                <div style={{ fontSize: 12.5, fontWeight: 600, color: C.amber, marginTop: 14 }}>Editando comprobante {editingAsiento.id} — {editingAsiento.glosa}</div>
                <AsientoForm key={editingAsiento.id} cuentas={cuentas} centros={centros} initial={editingAsiento} onSubmit={(data) => { updateAsiento(editingAsiento.id, data); setEditingAsiento(null); }} />
                <button onClick={() => setEditingAsiento(null)} style={{ fontSize: 12, color: C.muted, marginTop: 6, textDecoration: 'underline' }}>Cancelar edición</button>
              </div>
            )}

            <div className="rounded-xl overflow-hidden mt-5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              <table>
                <thead><tr><th>Fecha</th><th>Glosa</th><th>Centro de costo</th><th>Cuenta debe</th><th>Debe</th><th>Cuenta haber</th><th>Haber</th><th>Usuario</th><th>Acciones</th></tr></thead>
                <tbody>
                  {comprobantesPag.paginaActual.map((a) => (
                    <tr key={a.id} style={{ backgroundColor: editingAsiento?.id === a.id ? '#FFF7E8' : 'transparent', opacity: a.anulado ? 0.5 : 1, textDecoration: a.anulado ? 'line-through' : 'none' }}>
                      <td>{a.fecha}</td>
                      <td style={{ fontSize: 12.5 }}>{a.glosa} {a.anulado && <span className="text-xs font-bold px-1.5 py-0.5 rounded" style={{ backgroundColor: '#FDE8E8', color: C.coral, textDecoration: 'none', display: 'inline-block' }}>ANULADO</span>}</td>
                      <td style={{ fontSize: 12 }}>{a.centroCostoId ? centroCostoName(a.centroCostoId) : '—'}</td>
                      <td style={{ fontSize: 12 }}>{a.cuentaDebe} — {cuentaNombre(a.cuentaDebe)}</td>
                      <td style={{ fontFamily: 'JetBrains Mono, monospace' }}>{fmt(a.montoDebe)}</td>
                      <td style={{ fontSize: 12 }}>{a.cuentaHaber} — {cuentaNombre(a.cuentaHaber)}</td>
                      <td style={{ fontFamily: 'JetBrains Mono, monospace' }}>{fmt(a.montoHaber)}</td>
                      <td style={{ fontSize: 11, color: C.muted }}>{a.usuario || '—'}</td>
                      <td>
                        {!a.anulado && (
                          <div className="flex gap-2">
          ...
          <div>
            ...
            {contabVista === 'estado_resultados' && erModo === 'periodo' && (
              <div className="rounded-xl p-5 mt-6" style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, maxWidth: 620 }}>
                <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 600, fontSize: 14, color: C.slate, marginBottom: 4 }}>
                ...
                {erLineas.map((linea, i) => (
                  <div key={i} style={{ marginTop: 8 }}>
                    ...
                    {linea.cuentas.length === 0 && <div style={{ marginLeft: 14, fontSize: 11.5, color: C.amber }}>Sin cuentas asignadas a este grupo — ve a Plan de Cuentas para configurarlo.</div>}
                  </div>
                ))}
        ...
        {tab === 'contab_config' && (
          <div>
            ...
            <div className="rounded-xl overflow-hidden mt-3" style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, maxWidth: 520 }}>
              <table>
                ...
                <tbody>
                  {catalogoPartidas.map((p) => (
                    <tr key={p.id}>
                      <td><input value={p.nombre} onChange={(e) => setCatalogoPartidas((prev) => prev.map((x) => x.id === p.id ? { ...x, nombre: e.target.value } : x))} style={{ ...inputStyle, fontSize: 12.5, padding: '4px 6px', width: 220 }} /></td>
                      <td><input type="number" value={p.precioRef} onChange={(e) => setCatalogoPartidas((prev) => prev.map((x) => x.id === p.id ? { ...x, precioRef: Number(e.target.value) || 0 } : x))} style={{ ...inputStyle, fontSize: 12.5, padding: '4px 6px', width: 100, fontFamily: 'JetBrains Mono, monospace' }} /></td>
                      <td><button onClick={() => setCatalogoPartidas((prev) => prev.filter((x) => x.id !== p.id))}><Trash2 size={14} color={C.coral} /></button></td>
                    </tr>
                  ))}
              ...
              <div className="p-3">
                <button onClick={() => setCatalogoPartidas((prev) => [...prev, { id: nextId('PT', prev), nombre: 'Nueva partida', precioRef: 0 }])} className="px-3 py-1.5 rounded-lg text-xs font-semibold" style={{ backgroundColor: '#EEF1F4', color: C.slate }}>+ Agregar partida</button>
              </div>
            </div>
            ...
            <div className="rounded-xl p-4 mt-3" style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, maxWidth: 520 }}>
              <div className="flex flex-wrap gap-2">
                {tiposServicio.map((t) => (
                  <span key={t} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg" style={{ backgroundColor: '#F6F8FA', fontSize: 12.5 }}>
                    {t}
                    {tiposServicio.length > 1 && <button onClick={() => setTiposServicio((prev) => prev.filter((x) => x !== t))}><X size={12} color={C.coral} /></button>}
                  </span>
                ))}
              </div>
              <div className="flex gap-2 items-end mt-3">
                ...
                <button onClick={() => { const v = nuevoTipoServicio.trim(); if (v && !tiposServicio.includes(v)) { setTiposServicio((prev) => [...prev, v]); setNuevoTipoServicio(''); } }} className="px-3 py-2 rounded-lg text-xs font-semibold" style={{ backgroundColor: C.navy, color: '#fff' }}>Agregar</button>
              </div>
            </div>
            </>)}
                ...
                <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 18, paddingTop: 16 }}>
                  ...
                  <div className="flex flex-wrap gap-3 items-end mt-4">
                    ...
                    <button onClick={() => {
                      ...
                      const detalle = centros.filter((c) => c.id !== 'CC-ADM').map((c) => {
        ...
        {tab === 'agenda' && (() => {
          ...
          return (
            <div>
              ...
              <div className="rounded-xl overflow-hidden mt-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, overflowX: 'auto' }}>
              ...
              {agendaSel && (() => {
                ...
                const guardar = () => {
                  ...
                  setOrders((prev) => prev.map((x) => x.id === o.id ? {
                    ...
                    reprogramaciones: cambioFechaHora
                      ? [...(x.reprogramaciones || []), { de: `${o.fecha}${o.hora ? ' ' + o.hora : ''}`, a: `${agendaDraft.fecha}${agendaDraft.hora ? ' ' + agendaDraft.hora : ''}`, registrado: new Date().toISOString().slice(0, 16).replace('T', ' '), por: sesion.nombre }]
                      : x.reprogramaciones,
                  } : x));
                  setAgendaEdit(false);
                };
                    ...
                    <div onClick={(e) => e.stopPropagation()} className="rounded-2xl p-5" style={{ backgroundColor: '#fff', width: 440, maxWidth: '100%', maxHeight: '85vh', overflowY: 'auto', boxShadow: '0 20px 50px rgba(0,0,0,0.25)' }}>
                      ...
                      {!agendaEdit ? (
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: C.muted, marginTop: 10 }}>{t('Los montos y el estado se gestionan desde la pestaña Órdenes.')}</div>
                          <div className="flex gap-2 mt-4">
          ...
          <div>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p style={{ color: C.muted, fontSize: 13, marginTop: 2 }}>Cada persona entra con su usuario y clave, y solo ve los módulos de su rol.</p>
              </div>
              <button onClick={() => setShowUsuarioForm(!showUsuarioForm)} className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-white text-sm font-semibold" style={{ backgroundColor: C.navy }}>
                {showUsuarioForm ? <X size={15} /> : <Plus size={15} />} {showUsuarioForm ? 'Cerrar' : 'Nuevo usuario'}
              </button>
            </div>

            {showUsuarioForm && <UsuarioForm techs={techs} roles={rolesLista} onSubmit={(data) => {
              if (usuarios.some((u) => u.usuario === data.usuario)) { alert('Ese nombre de usuario ya existe.'); return; }
              setUsuarios((p) => [...p, { id: nextId('U', p), ...data, empresas: data.empresas && data.empresas.length ? data.empresas : [empresaActiva] }]);
              setShowUsuarioForm(false);
            }} />}

            <div className="rounded-xl overflow-hidden mt-5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
              <table>
                <thead><tr><th>Nombre</th><th>Usuario</th><th>Rol</th><th>Técnico vinculado</th><th>Empresas con acceso</th><th>Clave</th><th>Último acceso</th><th></th></tr></thead>
                <tbody>
                  {usuarios.map((u) => (
                    <tr key={u.id}>
                      <td style={{ fontWeight: 600 }}>{u.nombre}{u.id === sesion.id && <span style={{ fontSize: 10.5, color: C.cyanDark, marginLeft: 6 }}>(tú)</span>}</td>
                      <td style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12 }}>@{u.usuario}</td>
                      <td>
                        <select value={u.rol} onChange={(e) => setUsuarios((prev) => prev.map((x) => x.id === u.id ? { ...x, rol: e.target.value } : x))} disabled={u.id === sesion.id} style={{ ...inputStyle, fontSize: 12, padding: '4px 6px' }}>
                          {rolesLista.map((r) => <option key={r} value={r}>{r}</option>)}
                        </select>
                      </td>
                      <td>
                        {u.rol === 'Tecnico' ? (
                          <select value={u.empleadoId || ''} onChange={(e) => setUsuarios((prev) => prev.map((x) => x.id === u.id ? { ...x, empleadoId: e.target.value || null } : x))} style={{ ...inputStyle, fontSize: 12, padding: '4px 6px' }}>
                      ...
                      <td>
                        {u.rol === 'Administrador' ? <span style={{ fontSize: 11.5, color: C.cyanDark, fontWeight: 600 }}>Todas</span> : (
                          <div className="flex flex-col gap-0.5">
                            {empresas.map((emp) => (
                              <label key={emp.id} className="flex items-center gap-1.5" style={{ fontSize: 11.5, color: C.slate }}>
                                <input type="checkbox" checked={u.empresas === 'todas' || (u.empresas || []).includes(emp.id)}
                                  onChange={(ev) => setUsuarios((prev) => prev.map((x) => {
                                    if (x.id !== u.id) return x;
                                    const actual = x.empresas === 'todas' ? empresas.map((z) => z.id) : (x.empresas || []);
                                    return { ...x, empresas: ev.target.checked ? [...actual.filter((z) => z !== emp.id), emp.id] : actual.filter((z) => z !== emp.id) };
                                  }))}
                                  style={{ width: 13, height: 13, accentColor: C.cyanDark }} />
                                {emp.nombre}
                              </label>
                            ))}
                          </div>
                      ...
                      <td><input type="text" value={u.clave} onChange={(e) => setUsuarios((prev) => prev.map((x) => x.id === u.id ? { ...x, clave: e.target.value } : x))} style={{ ...inputStyle, fontSize: 12, padding: '4px 6px', width: 110, fontFamily: 'JetBrains Mono, monospace' }} /></td>
                      <td style={{ fontSize: 11.5, color: C.muted }}>{u.ultimoAcceso || 'Nunca'}{u.accesos ? ` · ${u.accesos} sesión(es)` : ''}</td>
                      <td>{u.id !== sesion.id && <button onClick={() => pedirConfirmacion(`¿Eliminar al usuario ${u.nombre}?`, () => setUsuarios((prev) => prev.filter((x) => x.id !== u.id)))}><Trash2 size={14} color={C.coral} /></button>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                    ...
                    <tr>
                      ...
                      {rolesLista.map((r) => (
                        <th key={r} style={{ textAlign: 'center' }}>
                          {r}
                          {r !== 'Administrador' && !['Contabilidad', 'Operaciones', 'Tecnico'].includes(r) && !usuarios.some((u) => u.rol === r) && (
                            <button onClick={() => pedirConfirmacion(`¿Eliminar el rol ${r}?`, () => setRolesConfig((prev) => { const n = { ...prev }; delete n[r]; return n; }))} title="Eliminar rol" style={{ marginLeft: 4 }}><Trash2 size={11} color={C.coral} /></button>
                          )}
                        </th>
                      ))}
                    </tr>
          ...
          <div>
            ...
            <div className="rounded-xl p-5 mt-5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, maxWidth: 480 }}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <div style={{ fontWeight: 700, color: C.slate }}>@{sesion.usuario}</div>
              ...
              <button onClick={() => {
                ...
                setUsuarios((prev) => prev.map((u) => u.id === sesion.id ? { ...u, nombre: cuentaNombreEdit.trim(), clave: cuentaClaveNueva ? cuentaClaveNueva : u.clave } : u));
                setSesion((prev) => ({ ...prev, nombre: cuentaNombreEdit.trim() }));
              ...
              <Field label="Pantalla de inicio al entrar">
                <select style={{ ...inputStyle, width: '100%' }} value={sesion.preferencias?.pantallaInicio || 'dashboard'}
                  onChange={(e) => {
                    ...
                    setUsuarios((prev) => prev.map((u) => u.id === sesion.id ? { ...u, preferencias: { ...u.preferencias, pantallaInicio: val } } : u));
              ...
              <div style={{ marginTop: 16 }}>
                <Field label="Símbolo de moneda para mostrar montos">
                  <select style={{ ...inputStyle, width: '100%' }} value={sesion.preferencias?.moneda || '$'}
                    onChange={(e) => {
                      const val = e.target.value;
                      setUsuarios((prev) => prev.map((u) => u.id === sesion.id ? { ...u, preferencias: { ...u.preferencias, moneda: val } } : u));
              ...
              <div style={{ marginTop: 16 }}>
                <Field label="Idioma del sistema">
                  <select style={{ ...inputStyle, width: '100%' }} value={sesion.preferencias?.idioma || 'es'}
                    onChange={(e) => {
                      const val = e.target.value;
                      setUsuarios((prev) => prev.map((u) => u.id === sesion.id ? { ...u, preferencias: { ...u.preferencias, idioma: val } } : u));
  ...
  return (
    <div className="rounded-xl p-4 mt-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <Field label="Concepto"><input style={{ ...inputStyle, width: 200 }} value={concepto} onChange={(e) => setConcepto(e.target.value)} /></Field>
        {tipo === 'gasto' ? (
      ...
      {tipo === 'inventario' && <div style={{ fontSize: 11, color: C.muted, marginTop: 8 }}>Al guardar, esta compra aumenta el stock del material elegido y recalcula su costo promedio ponderado — se contabiliza a Inventario (1435), no como gasto.</div>}
      <button onClick={() => { if (concepto && monto !== '' && (tipo === 'gasto' || (materialId && cantidad !== ''))) onSubmit({ proveedorId, concepto, tipo, categoria: tipo === 'gasto' ? categoria : null, materialId: tipo === 'inventario' ? materialId : null, cantidad: tipo === 'inventario' ? cantidad : null, fecha, vencimiento, monto }); }}
...
function UsuarioForm({ techs, roles, onSubmit }) {
  const [nombre, setNombre] = useState('');
  ...
  return (
    <div className="rounded-xl p-4 mt-4 flex flex-wrap gap-3 items-end" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
      <Field label="Nombre completo"><input style={{ ...inputStyle, width: 200 }} value={nombre} onChange={(e) => setNombre(e.target.value)} /></Field>
      <Field label="Usuario"><input style={{ ...inputStyle, width: 140 }} value={usuario} onChange={(e) => setUsuario(e.target.value.toLowerCase().replace(/\s/g, ''))} placeholder="sin espacios" /></Field>
      <Field label="Clave"><input style={{ ...inputStyle, width: 140 }} value={clave} onChange={(e) => setClave(e.target.value)} /></Field>
      <Field label="Rol">
        <select style={{ ...inputStyle, width: 160 }} value={rol} onChange={(e) => setRol(e.target.value)}>
          {roles.map((r) => <option key={r} value={r}>{r}</option>)}
        </select>
      </Field>
      {rol === 'Tecnico' && (
        <Field label="Técnico vinculado">
          <select style={{ ...inputStyle, width: 180 }} value={empleadoId} onChange={(e) => setEmpleadoId(e.target.value)}>
            <option value="">— Sin vincular —</option>
            {techs.map((t) => <option key={t.id} value={t.id}>{t.nombre}</option>)}
          </select>
        </Field>
      )}
      <button onClick={() => { if (nombre.trim() && usuario.trim() && clave.trim()) onSubmit({ nombre: nombre.trim(), usuario: usuario.trim(), clave, rol, empleadoId: empleadoId || null, empresas: [], preferencias: { pantallaInicio: 'dashboard', moneda: '$', idioma: 'es' } }); }} className="px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ backgroundColor: C.cyanDark }}>
        Crear usuario
      </button>
    </div>
  );
  ...
  return (
    <div className="rounded-xl p-4 mt-4" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
      <div className="flex flex-wrap gap-3 items-end mb-3">
        <Field label="Fecha"><input type="date" style={inputStyle} value={fecha} onChange={(e) => setFecha(e.target.value)} /></Field>
        <Field label="Glosa (descripción)"><input style={{ ...inputStyle, width: 280 }} value={glosa} onChange={(e) => setGlosa(e.target.value)} /></Field>
      ...
      <div className="flex flex-wrap gap-3 items-end">
        <Field label="Cuenta debe">
          <select style={{ ...inputStyle, width: 240 }} value={cuentaDebe} onChange={(e) => setCuentaDebe(e.target.value)}>
            {cuentas.map((c) => <option key={c.codigo} value={c.codigo}>{c.codigo} — {c.nombre}</option>)}
          </select>
        </Field>
        <Field label="Monto debe ($)"><input type="number" style={{ ...inputStyle, width: 120 }} value={montoDebe} onChange={(e) => { setMontoDebe(e.target.value); setMontoHaber(e.target.value); }} /></Field>
        <Field label="Cuenta haber">
          <select style={{ ...inputStyle, width: 240 }} value={cuentaHaber} onChange={(e) => setCuentaHaber(e.target.value)}>
            {cuentas.map((c) => <option key={c.codigo} value={c.codigo}>{c.codigo} — {c.nombre}</option>)}
          </select>
        </Field>
        <Field label="Monto haber ($)"><input type="number" style={{ ...inputStyle, width: 120 }} value={montoHaber} onChange={(e) => setMontoHaber(e.target.value)} /></Field>
        <button onClick={() => { if (glosa && montoDebe !== '') onSubmit({ fecha, glosa, cuentaDebe, montoDebe, cuentaHaber, montoHaber, centroCostoId: centroCostoId || null }); }} className="px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ backgroundColor: C.cyanDark }}>{initial ? 'Guardar cambios' : 'Registrar asiento'}</button>
      </div>
      <div style={{ fontSize: 11, color: C.muted, marginTop: 8 }}>Partida simple: un cargo (debe) y un abono (haber). El monto se copia igual en ambos lados; ajústalo si el asiento lo requiere.</div>
    </div>
  );
}
...
function SalidaForm({ materials, defaultMaterial, centros, onSubmit }) {
  const [materialId, setMaterialId] = useState(defaultMaterial);
  const [cantidad, setCantidad] = useState('');
  const [fecha, setFecha] = useState(new Date().toISOString().slice(0, 10));
  const [referencia, setReferencia] = useState('');
  ...
  return (
    <div className="rounded-xl p-4 mt-4 flex flex-wrap gap-3 items-end" style={{ backgroundColor: C.card, border: `1px solid ${C.border}` }}>
      ...
      <button onClick={() => { if (materialId && cantidad !== '' && Number(cantidad) > 0) { onSubmit({ materialId, cantidad, fecha, referencia, centroCostoId }); setCantidad(''); setReferencia(''); } }}
        className="px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ backgroundColor: C.coral }}>
        Registrar salida
      </button>
      <div style={{ fontSize: 11, color: C.muted, width: '100%' }}>Úsalo para consumos que no vienen de una orden de trabajo (merma, uso interno, ajustes). Descuenta stock y contabiliza el costo en el centro elegido.</div>
    </div>
  );
}
