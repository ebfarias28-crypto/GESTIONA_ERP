...
export default function App() {
  ...
  const invitarUsuario = async () => {
    ...
    setInviteEmail('');
    alert(`Invitación enviada a ${email}. En cuanto esa persona cree su cuenta (o inicie sesión, si ya la tiene) con ese mismo correo, obtendrá acceso automáticamente.`);
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: miembrosData } = await supabase.from('perfil_empresas').select('perfil_id, rol_id, perfiles(nombre, email, ultimo_acceso, empleado_id), roles(nombre)').eq('empresa_id', empresaActiva);
      setMiembrosEmpresa((miembrosData || []).map((m) => ({ perfilId: m.perfil_id, nombre: m.perfiles?.nombre, email: m.perfiles?.email, ultimoAcceso: m.perfiles?.ultimo_acceso, empleadoId: m.perfiles?.empleado_id, rol: m.roles?.nombre, rolId: m.rol_id })));
      ...
      eq('ordenes').then(({ data }) => setOrders((data || []).map((o) => ({
        ...
        validada: o.validada, validadaPor: o.validada_por, fechaValidacion: o.fecha_validacion, capturadaPor: o.capturada_por,
      ...
      const mapeados = (asData || []).map((a) => ({
        ...
        centroCostoId: a.centro_costo_id, anulado: a.anulado, usuario: a.usuario_id,
      }));
  ...
  const notificar = async (destinatarios, tipo, titulo, mensaje, ordenId) => {
    ...
    const { data: insertadas, error } = await supabase.from('notificaciones').insert(filas).select();
    if (error) { console.error('Error al guardar notificación:', error.message); return; }
    setNotificaciones((prev) => {
      const nuevas = (insertadas || []).map((n) => ({ id: n.id, usuarioId: n.usuario_id, tipo: n.tipo, titulo: n.titulo, mensaje: n.mensaje, ordenId, fecha: n.creada_en, leida: false }));
  ...
  const registerSalida = async ({ materialId, cantidad, fecha, referencia, centroCostoId }) => {
    const idx = materials.findIndex((m) => m.id === materialId);
    if (idx === -1) return;
    const m = materials[idx];
  ...
  const addCxp = async (data) => {
    ...
    const { data: nuevo, error } = await supabase.from('cxp').insert({
      ...
      fecha: data.fecha, vencimiento: data.vencimiento, monto, estado: 'pendiente',
    }).select().single();
    ...
    if (data.tipo === 'inventario' && data.materialId && Number(data.cantidad) > 0) {
      ...
      if (idx !== -1) {
        ...
        setMaterials(matActualizados);
        await supabase.from('materiales').update({ stock: nuevoStock, costo_unitario: nuevoCostoProm }).eq('id', data.materialId);
        const { data: kNuevo } = await supabase.from('kardex').insert({
          empresa_id: empresaActiva, material_id: data.materialId, fecha: data.fecha, tipo: 'entrada', cantidad, costo_unitario: costoUnitario,
          referencia: `Compra a crédito ${nuevo.id}`, saldo_cantidad: nuevoStock, saldo_valor: nuevoStock * nuevoCostoProm, centro_costo_id: null,
        }).select().single();
        if (kNuevo) setKardexLog((prev) => [...prev, {
          id: kNuevo.id, materialId: data.materialId, fecha: data.fecha, tipo: 'entrada', cantidad, costoUnitario,
          referencia: `Compra a crédito ${nuevo.id}`, saldoCantidad: nuevoStock, saldoValor: nuevoStock * nuevoCostoProm, centroCostoId: 'CC-ADM',
        }]);
      }
        ...
        {tab === 'usuarios' && sesion.rol === 'Administrador' && (
          <div>
            <div>
              ...
              <p style={{ color: C.muted, fontSize: 13, marginTop: 2 }}>Invita por correo a quien necesite acceso a esta empresa — se agrega automáticamente en cuanto cree su cuenta o inicie sesión con ese mismo correo.</p>
            </div>
              ...
              <table>
                ...
                <tbody>
                  {miembrosEmpresa.map((m) => (
                    <tr key={m.perfilId}>
                      <td style={{ fontWeight: 600 }}>{m.nombre || '—'}{m.perfilId === sesion.id && <span style={{ fontSize: 10.5, color: C.cyanDark, marginLeft: 6 }}>(tú)</span>}</td>
                  ...
                  {miembrosEmpresa.length === 0 && <tr><td colSpan={5} style={{ color: C.muted, textAlign: 'center', padding: 20 }}>Sin miembros todavía.</td></tr>}
                </tbody>
              </table>
            </div>
          ...
          <div>
            ...
            <div className="rounded-xl p-5 mt-5" style={{ backgroundColor: C.card, border: `1px solid ${C.border}`, maxWidth: 480 }}>
              ...
              <button onClick={async () => {
                ...
                const { error: errPerfil } = await supabase.from('perfiles').update({ nombre: cuentaNombreEdit.trim() }).eq('id', sesion.id);
                if (errPerfil) { setCuentaMsg('No se pudo guardar el nombre: ' + errPerfil.message); return; }
                if (cuentaClaveNueva) {
